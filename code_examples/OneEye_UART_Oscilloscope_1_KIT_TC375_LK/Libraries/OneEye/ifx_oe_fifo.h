...
#ifndef IFX_OE_FIFO_H
#define IFX_OE_FIFO_H
//------------------------------------------------------------------------------
#include "ifx_oe_al.h"
...
typedef struct
{
    Ifx_Oe_SizeT count;            /**< \brief number of bytes contained in the buffer */
    ...
    Ifx_Oe_SizeT maxcount;         /**< \brief Highest value seen in the count */
} Ifx_Oe_Fifo_Shared;
 ...
 *
 */
typedef struct _Ifx_Oe_Fifo
{
    void* buffer;                           /**< \brief aligned on 64 bit boundary */
    Ifx_Oe_Fifo_Shared shared;                 /**< \brief  data shared between reader / writer */
    Ifx_Oe_SizeT startIndex;                   /**< \brief buffer valid data start index */
    Ifx_Oe_SizeT endIndex;                     /**< \brief buffer valid data end index */
    Ifx_Oe_SizeT size;                         /**< \brief multiple of 8 bit, max 0xFFF8 */
    Ifx_Oe_SizeT elementSize;                  /**< \brief minimum number of bytes (block) added / removed to / from the buffer */
 ...
 */
IFX_OE_EXTERN boolean Ifx_Oe_Fifo_canReadCount(Ifx_Oe_Fifo* fifo, Ifx_Oe_SizeT count, Ifx_Oe_TickTime timeout);
 ...
 */
IFX_OE_EXTERN boolean Ifx_Oe_Fifo_canWriteCount(Ifx_Oe_Fifo* fifo, Ifx_Oe_SizeT count, Ifx_Oe_TickTime timeout);
 ...
 * \return void
 */
IFX_OE_EXTERN void Ifx_Oe_Fifo_clear(Ifx_Oe_Fifo* fifo);
 ...
 *
 * \see Ifx_Oe_Fifo_destroy()
 */
IFX_OE_EXTERN Ifx_Oe_Fifo* Ifx_Oe_Fifo_create(Ifx_Oe_SizeT size, Ifx_Oe_SizeT elementSize);

/** \brief Destroy the FIFO object
 *
 * This function must be called to destroy the fifo object when created with \ref Ifx_Oe_Fifo_create()
 *
 * \param fifo Pointer on the Fifo object
 * \return void
 *
 * \see   Ifx_Oe_Fifo_create()
 */
IFX_OE_EXTERN void Ifx_Oe_Fifo_destroy(Ifx_Oe_Fifo* fifo);
 ...
 * equals to "size + sizeof(Ifx_Oe_Fifo) + 8". Not taking this in account may result
 * in unpredictable behavior.
 */
IFX_OE_EXTERN Ifx_Oe_Fifo* Ifx_Oe_Fifo_init(void* buffer, Ifx_Oe_SizeT size, Ifx_Oe_SizeT elementSize);
 ...
 * elementSize then the incomplete element is not removed from the buffer.
 *
 * \param fifo Pointer on the Fifo object
 * \param count in bytes
 *
 * \return return the number of byte that could not be removed
 */
IFX_OE_EXTERN Ifx_Oe_SizeT Ifx_Oe_Fifo_remove(Ifx_Oe_Fifo* fifo, Ifx_Oe_SizeT count);
 ...
 * \param timeout in system timer ticks
 *
 * @return Return the count in byte not written to the buffer. If 0, all bytes were written
 */
IFX_OE_EXTERN Ifx_Oe_SizeT Ifx_Oe_Fifo_write(Ifx_Oe_Fifo* fifo, const void* data, Ifx_Oe_SizeT count, Ifx_Oe_TickTime timeout);
 ...
 */
IFX_OE_INLINE boolean Ifx_Oe_Fifo_flush(Ifx_Oe_Fifo* fifo, Ifx_Oe_TickTime timeout)
{
    return Ifx_Oe_Fifo_canWriteCount(fifo, fifo->size, timeout);
}
 ...
 * \param fifo Pointer on the Fifo object
 *
 * Note as the Ifx_Oe_Fifo_write / Ifx_Oe_Fifo_read function does only write blocks which are
 * a multiple of fifo->elementSize, the Ifx_Oe_Fifo_readCount / Ifx_Oe_Fifo_writeCount return
 ...
 */
IFX_OE_INLINE Ifx_Oe_SizeT Ifx_Oe_Fifo_writeCount(Ifx_Oe_Fifo* fifo)
{
    return (Ifx_Oe_SizeT)(fifo->size - Ifx_Oe_Fifo_readCount(fifo));
}


/** \brief Indicates if the fifo is empty
 *
 * \param fifo Pointer on the Ifx_Oe_Fifo object
 *
 * \retval TRUE is the buffer is empty
 * \retval FALSE is the buffer is not empty
 */
IFX_OE_INLINE boolean Ifx_Oe_Fifo_isEmpty(Ifx_Oe_Fifo* fifo)
{
    return (Ifx_Oe_Fifo_readCount(fifo) != 0) ? FALSE : TRUE;
}
 ...
 * \param fifo Pointer on the Ifx_Oe_Fifo object
 *
 * \retval TRUE is the buffer is full
 * \retval FALSE is the buffer is not full
 */
IFX_OE_INLINE boolean Ifx_Oe_Fifo_isFull(Ifx_Oe_Fifo* fifo)
{
    return (Ifx_Oe_Fifo_writeCount(fifo) != 0) ? FALSE : TRUE;
}
 ...
 *
 *
 * \retval Return the FIFO size
 */
IFX_OE_INLINE Ifx_Oe_SizeT Ifx_Oe_Fifo_size(Ifx_Oe_Fifo* fifo)
{
    return fifo->size;
}
 ...
 *
 *
 * \retval Return a pointer to the next data to be read
 */
IFX_OE_INLINE char* Ifx_Oe_Fifo_startDataPointer(Ifx_Oe_Fifo* fifo)
{
    return &((char*)fifo->buffer)[fifo->startIndex];
}


/** \brief End data pointer
 *
 *
 * \retval Return a pointer to the next data to be written
 */
