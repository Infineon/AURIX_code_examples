/**********************************************************************************************************************
 * \file TLE5012BD_SENT_SPC_Redundancy.c
...
/*------------------------------------------------------Macros-------------------------------------------------------*/
/*********************************************************************************************************************/
#define PWM_PERIOD                          3906              /* PWM period for the TOM i.e. 100Hz                   */
...
#define THRESHOLD_VALUE                     5                 /* Threshold value, above this LED as alarm will be on */
#define NUMBER_OF_NIBBLES_FOR_CRC           4                 /* Total number of nibbles used for CRC calculation    */
#define TLE5012B_SPC_FRAME_CRC_SEED_VAL     0x05              /* the seed value of the fast CRC circuit from user
                                                                 manual of TLE5012 sensor */
/* LED on Application Kit */
#define LED_D107_ALARM                      &MODULE_P13, 0    /* LED D107: Port, Pin definition                      */

/* Angle Sensor 1 define */
#define SENSOR_ID1                    24              /* sensor ID which is actually SPC pulse length 24 * 3 = 72 µs */
#define SENT_CH4B_PIN_IN              IfxSent_SENT4B_P00_5_IN         /* SENT input pin                              */
#define SPC_CH4_PIN_OUT               IfxSent_SPC4_P00_5_OUT          /* SPC output pin                              */
#define SPC_PULSE_CH4B_TRIGGER_INPUT  IfxGtm_TOM2_6N_TOUT86_P14_6_OUT /* TOM output pin to trigger SPC pulse         */

#define ISR_PRIORITY_SENT_CHANNEL4    4                             /* Interrupt Priority of sensor 1                */

/* Angle Sensor 2 defines */
#define SENSOR_ID2                    24              /* sensor ID which is actually SPC pulse length 24 * 3 = 72 µs */
#define SENT_CH9B_PIN_IN              IfxSent_SENT9B_P00_10_IN        /* SENT input pin                              */
#define SPC_CH9_PIN_OUT               IfxSent_SPC9_P00_10_OUT         /* SPC output pin                              */
#define SPC_PULSE_CH9B_TRIGGER_INPUT  IfxGtm_TOM0_6_TOUT59_P20_0_OUT  /* TOM output pin to trigger SPC pulse         */

#define ISR_PRIORITY_SENT_CHANNEL9    5                               /* Interrupt Priority of sensor 2              */

/*********************************************************************************************************************/
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
DataTle5012 g_dataTle5012;                          /* global variable for sent struct                               */
static SentErrorCounters g_errorCounters = {0};     /* error count                                                   */
static float32 g_angleCoveredDifference;            /* angle difference between two sensor                           */
static boolean g_alarmFlag = FALSE;                 /* alarm flag                                                    */

IfxGtm_Tom_Pwm_Driver g_tomDriver[2];               /* Timer Driver structure, 2 different instance of Tom are used  */

/*********************************************************************************************************************/
/*------------------------------------------------Function Prototypes------------------------------------------------*/
/*********************************************************************************************************************/
void interruptHandlerSENT(DataTle5012 *sent, uint8 instance);
void initSentCh4bSpcMode(void);
void initSentCh9bSpcMode(void);
void initGtmTom2Ch7TriggerInput(void);
void initGtmTom0Ch7TriggerInput(void);
void initSENTmoduleForTle5012(void);
void initGTMTOM(void);
void initLED(void);
void crcCalculation(uint8 instance, uint8 channelId);
uint8 calculateCrcTle5012SPC(uint8 *message, uint8 length);

/*********************************************************************************************************************/
/*---------------------------------------------Function Implementations----------------------------------------------*/
/*********************************************************************************************************************/
IFX_INTERRUPT(channel4SENTisr, 0, ISR_PRIORITY_SENT_CHANNEL4);
IFX_INTERRUPT(channel9SENTisr, 0, ISR_PRIORITY_SENT_CHANNEL9);

/* SENT CH4 interrupt */
void channel4SENTisr(void)
{
    interruptHandlerSENT(&g_dataTle5012, 0);
}

/* SENT CH9 interrupt */
void channel9SENTisr(void)
{
    interruptHandlerSENT(&g_dataTle5012, 1);
}
...
void interruptHandlerSENT(DataTle5012 *sent, uint8 instance)
{
    ...
    if(interruptStatus.U)
    {
        ...
        if (interruptStatus.B.RSI)
        {
            ...
            /* crc Calculation */
            crcCalculation(instance, sent->sentChannel[instance].channelId);

            /*raw data conversion to angle in degree i.e. max 360 */
            sent->sentAngle[instance] = (float32)(sent->sentRawData[instance].U * TLE5012B_SPC_ANGLE_RES_12b);

            /* apply angle offset value to sensor 2 only */
            if (instance == 1)
            {
             sent->sentAngle[instance] = sent->sentAngle[instance]- ANGLE_OFFSET;
            }
            /* increment counter */
...
void crcCalculation(uint8 instance, uint8 channelId)
{
    uint8 receivedData[NUMBER_OF_NIBBLES_FOR_CRC] = {0, 0, 0, 0};
    /* read received CRC value from sensor */
    g_dataTle5012.sentCrcReceived[instance] = IfxSent_readReceivedCrc(g_dataTle5012.sentModule.sent, channelId);

    /* swap data as sent by tle5012 sensor and add status because it is included in CRC calculation on sensor side*/
    receivedData[0] = g_dataTle5012.sentStatus[instance];
    receivedData[1] = g_dataTle5012.sentRawData[instance].B.nibble2;
    receivedData[2] = g_dataTle5012.sentRawData[instance].B.nibble1;
    receivedData[3] = g_dataTle5012.sentRawData[instance].B.nibble0;

    /* calculate CRC on the received data */
    g_dataTle5012.sentCrcCalculated[instance] = calculateCrcTle5012SPC(&receivedData[0], NUMBER_OF_NIBBLES_FOR_CRC);
}
...
uint8 calculateCrcTle5012SPC(uint8 *message, uint8 length)
{
    uint8 crc = TLE5012B_SPC_FRAME_CRC_SEED_VAL;
...
/*
 * Initialization of GTM TOM to trigger SPC pulse for sensor 1
 */
void initGtmTom2Ch7TriggerInput(void)
{
...
/*
 * Initialization of SPC channel 4 for sensor 1
 */
void initSentCh4bSpcMode(void)
{
    ...
    /* initialize module */
    IfxSent_Sent_initModule(&g_dataTle5012.sentModule, &sentConfig);

    /* create channel config */
    IfxSent_Sent_ChannelConfig sentChannelConfig;
    IfxSent_Sent_initChannelConfig(&sentChannelConfig, &g_dataTle5012.sentModule);
    ...
    /* initialize channel */
    IfxSent_Sent_initChannel(&g_dataTle5012.sentChannel[0], &sentChannelConfig);
}

/*
 * Initialization of SPC channel 9 for sensor 2
 */
void initSentCh9bSpcMode(void)
{
    /* create module config */
    ...
    /* initialize channel */
    IfxSent_Sent_initChannel(&g_dataTle5012.sentChannel[1], &sentChannelConfig);
}
...
void initLED(void)
{
    ...
    IfxPort_setPinModeOutput(LED_D107_ALARM, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);

    /* Switch OFF the LED (low-level active) */
    IfxPort_setPinHigh(LED_D107_ALARM);
}
 ...
 */
void checkTle5012SENTredundancy()
{
    ...
    else
    {
         ...
         */
        if (g_dataTle5012.sentAngle[0] < START_ANGLE)
        {
            g_dataTle5012.sentAngleCovered[0] = START_ANGLE - g_dataTle5012.sentAngle[0];
            g_dataTle5012.sentAngleCovered[1] = g_dataTle5012.sentAngle[1] - START_ANGLE;
        }
        else if (g_dataTle5012.sentAngle[1] < START_ANGLE)
        {
            g_dataTle5012.sentAngleCovered[0] = 360.00f + START_ANGLE - g_dataTle5012.sentAngle[0];
            g_dataTle5012.sentAngleCovered[1] = 360.00f + g_dataTle5012.sentAngle[1] - START_ANGLE;
        }
        else
        {
            g_dataTle5012.sentAngleCovered[0] = 360.00 + START_ANGLE - g_dataTle5012.sentAngle[0];
            g_dataTle5012.sentAngleCovered[1] = g_dataTle5012.sentAngle[1] - START_ANGLE;
        }

        /* calculate minimum difference of two sensors */
        diff1 = fabsf(g_dataTle5012.sentAngleCovered[0] - g_dataTle5012.sentAngleCovered[1]);
        diff2 = 360 - diff1;
        g_angleCoveredDifference = fminf(diff1, diff2);

        /* If the difference is outside Threshold or CRC does not match */
        if ((g_angleCoveredDifference > THRESHOLD_VALUE)    ||
            (g_dataTle5012.sentCrcCalculated [0] != g_dataTle5012.sentCrcReceived[0])  ||
            (g_dataTle5012.sentCrcCalculated [1] != g_dataTle5012.sentCrcReceived[1]))
        {
            /* outside limit or CRC Mismatch */
            IfxPort_setPinLow(LED_D107_ALARM);
            g_alarmFlag = TRUE;
        }
        else
        {
