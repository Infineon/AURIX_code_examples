...
#include "phy/rtl8251b.h"

#define PHY_ID              0x001cc860
#define PHY_ID_MSK          0xfffffff0

#define PHY_RTL8251B_RESET_HOLD_TIME_MS 10
...
static int rtl8251b_set_serdes_option(const phy_t *phy)
{
    uint16_t reg_val;

    /* 5G link, 5000BASE-R */
    reg_val = phy->read(phy->addr, PHY_MMD_VEND1, 0x6973);
    reg_val &= (uint16_t)0xff00;
    reg_val |= (uint16_t)0x0004;
    phy->write(phy->addr, PHY_MMD_VEND1, 0x6973, reg_val);

    /* 2P5G link, 2500BASE-X */
    reg_val = phy->read(phy->addr, PHY_MMD_VEND1, 0x6974);
    reg_val &= (uint16_t)0xff00;
    reg_val |= (uint16_t)0x0005;
    phy->write(phy->addr, PHY_MMD_VEND1, 0x6974, 0x0005);

    /* 1G link, SGMII */
    reg_val = phy->read(phy->addr, PHY_MMD_VEND1, 0x6975);
    reg_val &= (uint16_t)0xff00;
    reg_val |= (uint16_t)0x0007;
    phy->write(phy->addr, PHY_MMD_VEND1, 0x6975, 0x0007);

    /* 100M link, SGMII */
    reg_val = phy->read(phy->addr, PHY_MMD_VEND1, 0x6976);
    reg_val &= (uint16_t)0xff00;
    reg_val |= (uint16_t)0x0007;
    phy->write(phy->addr, PHY_MMD_VEND1, 0x6976, 0x0007);

    /* 10M link, SGMII */
    reg_val = phy->read(phy->addr, PHY_MMD_VEND1, 0x6977);
    reg_val &= (uint16_t)0xff00;
    reg_val |= (uint16_t)0x0007;
    phy->write(phy->addr, PHY_MMD_VEND1, 0x6977, 0x0007);
...
static int rtl8251b_set_serdes_aneg(const phy_t *phy, bool enable)
{
...
static int rtl8251b_reset(const phy_t *phy)
{
    uint32_t phy_id;
    do
    {
    ...
    IfxStm_waitTicks(&MODULE_CPU0, IfxStm_getTicksFromMilliseconds(PHY_RTL8251B_RESET_HOLD_TIME_MS));
...
int rtl8251b_init(phy_t *phy, uint8 addr, phy_read_reg_t read, phy_write_reg_t write, uint32 init_flags)
{
    phy->addr = addr;
    phy->read = read;
    phy->write = write;
    ...
    if (rtl8251b_reset(phy) != 0)
    {
        return -1;
    }

    rtl8251b_set_serdes_option(phy);
    rtl8251b_set_serdes_aneg(phy, false);

    return 0;
}


int rtl8251b_update_link(phy_t *phy)
{
    if (phy->link_status.is_up || phy_aneg_is_complete(phy))
    {
        phy_link_state_t new_state = {};

        /* Read Phy registers to determine speed */
        uint16_t physr = phy->read(phy->addr, PHY_MMD_VEND2, PHYSR);
        ...
        if (new_state.is_up == true)
        {
            ...
            switch (speed)
            {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    new_state.speed = LINK_FULL_5000BASE_T;
                    break;
                default:
                    break;
            }
...
int rtl8251b_cfg_link(const phy_t *phy, enum phy_link_speed speeds)
{
    uint16_t reg_val;
    ...
    reg_val &= (uint16_t)~(PHY_MMD_AN_MULTIG_CONTROL1_2_5GBASE_T | PHY_MMD_AN_MULTIG_CONTROL1_5GBASE_T);
    reg_val |= (speeds & LINK_FULL_2500BASE_T ? PHY_MMD_AN_MULTIG_CONTROL1_2_5GBASE_T : 0);
    reg_val |= (speeds & LINK_FULL_5000BASE_T ? PHY_MMD_AN_MULTIG_CONTROL1_5GBASE_T : 0);
