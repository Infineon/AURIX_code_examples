...
#define ERASEVERIFY_ADDR            (WRITEPAGE_ADDR + WRITEPAGE_LEN)
#define ERASEPFLASH_ADDR            (ERASEVERIFY_ADDR + ERASEVERIFY_LEN)
...
typedef struct
{
    ...
    void (*eraseVerifySector)(uint32 sectorAddr);
...
void copyFunctionsToPSPR()
{
    ...
    /* Copy the IfxFlash_eraseVerifySector() routine and assign it to a function pointer */
    memcpy((void *)ERASEVERIFY_ADDR, (const void *)IfxFlash_eraseVerifySector, ERASEVERIFY_LEN);
    g_commandFromPSPR.eraseVerifySector = (void *)ERASEVERIFY_ADDR;
...
void writeProgramFlash()
{
    ...
    /* Verify erasure of the Flash sector before writing */
    g_commandFromPSPR.eraseVerifySector(PFLASH_STARTING_ADDRESS);

    /* Write the Program Flash */
    g_commandFromPSPR.writeFlash(PFLASH_STARTING_ADDRESS);

    /* Verify erasure of the Flash sector after writing */
    g_commandFromPSPR.eraseVerifySector(PFLASH_STARTING_ADDRESS);
