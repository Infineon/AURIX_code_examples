 ...
 *
 *      //Note: For DSP Luenberger Observer (FC5)
 *      //For LoLUT ICCM image use cdspFc5IccmImage array (defined in IfxAdcCdspFw_cfg.c file).
 *      //For LoLUT DCCM UCT, use User Configuration Table structure UserConfig_LoType to update the configuration parameters
 *      //const uint16       cdspFc5LoLutDccmSize = sizeof(UserConfig_LoType);
 *      //UserConfig_LoType cdspFc5LoLutDccm; // Update the cdspFc5LoLUTDccm structure as per application needs for CoeffA, CoeffB, CoeffC and other parameters.
 *      //IfxAdc_Cdsp_MemoryConfig memoryCfg = {&cdspFc5IccmImage, &cdspFc5LoLutDccm, cdspFc5IccmImageSize, cdspFc5LoLutDccmSize};
 *		//dspConfig.filterchainSel = IfxAdc_Cdsp_FilterChainSel_luenbergerObserver;
 *
 *
 ...
 *
 *      //Note: For custom Cdsp image
 *      //dspConfig.memCfg         = NULL;      //Memory Configuration
 *      //dspConfig.filterchainSel = IfxAdc_Cdsp_FilterChainSel_custom;
 *      //dspConfig.runCdspCore = FALSE;
 *      //After IfxAdc_Cdsp_initDspCore() is called, user has to load the custom ICCM image and custom DCCM UCT and then run the Dsp Core.
 *      //Steps to load custom image:
 *      //1. Reset Dsp Core using IfxAdc_resetCdspCore API.
 *      //2. Wait till the ARC core is in halt state (use IfxAdc_getCdspCoreHaltStatus API).
 *      //3. Clear CDSP ICCM and DCCM memory to avoid Ecc errors.
 *      //4. Load ICCM image in ICCM  memory using IfxAdc_loadCdspMemory API.
 *      //5. Load User Configuration Table(UCT) configuration in DCCM memory using IfxAdc_loadCdspMemory API.
 *      //6. Run Dsp Core using IfxAdc_runCdspCore API
 *
 * \endcode
 *
...
typedef enum
{
	...
	IfxAdc_Cdsp_FilterChainSel_custom                = 12  /**< \brief Custom implementation of DSP software */
 ...
 *
 * Note:By deafult, each filter chain is provided as a single binary image. The exception is FC5 (Luenberger Observer), which uses two binaries.
 * The filter chain binary image is loaded into ICCM memory, and the User configuration table(UCT) is loaded into the DCCM memory.
 * For FC5 - Luenberger observer (LO), there are two images: an ICCM binary and a DCCM LoLut binary. The ICCM binary is loaded into ICCM memory,
 * the User configuration table(UCT) is loaded into the DCCM memory, and the LoLut binary is loaded into DCCM memory at offset \ref IFXADC_CDSP_LOLUT_IMAGE_OFFSET.
 *
