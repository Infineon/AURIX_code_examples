...
/** \name Interrupt APIs
 * \{ */
IFX_INLINE boolean Ifx_areInterruptsEnabled(void);
IFX_INLINE boolean Ifx_disableInterrupts(void);
IFX_INLINE void    Ifx_enableInterrupts(void);
IFX_INLINE void    Ifx_restoreInterrupts(boolean enabled);
IFX_INLINE void    Ifx_forceDisableInterrupts(void);
/** \} */
/** \} */
...
/** \name Time APIs
 * \{ */
IFX_INLINE Ifx_TickTime Ifx_addTTime(Ifx_TickTime a, Ifx_TickTime b);
IFX_INLINE Ifx_TickTime Ifx_elapsed(Ifx_TickTime since);
IFX_INLINE Ifx_TickTime Ifx_getDeadLine(Ifx_TickTime timeout);
IFX_INLINE Ifx_TickTime Ifx_getTimeout(Ifx_TickTime deadline);
#if !defined(IFX_ILLD_PPU_USAGE)
IFX_EXTERN void         Ifx_initTime(void);
#endif
IFX_INLINE boolean      Ifx_isDeadLine(Ifx_TickTime deadLine);
IFX_INLINE Ifx_TickTime Ifx_now(void);
IFX_INLINE Ifx_TickTime Ifx_nowWithoutCriticalSection(void);
IFX_INLINE boolean      Ifx_poll(volatile boolean *test, Ifx_TickTime timeout);
IFX_INLINE Ifx_TickTime Ifx_timingNoInterruptEnd(Ifx_TickTime since, boolean interruptEnabled);
IFX_INLINE Ifx_TickTime Ifx_timingNoInterruptStart(boolean *interruptEnabled);
IFX_INLINE void         Ifx_wait(Ifx_TickTime timeout);
IFX_EXTERN void         Ifx_waitPoll(void);
IFX_EXTERN void         Ifx_waitTime(Ifx_TickTime timeout);
#if defined(IFX_ILLD_PPU_USAGE)
IFX_EXTERN Ifx_TickTime Ifx_getPpuTicks(uint32 microseconds);
#endif
...
#define TimeConst_10ns  (Ifx_TimeConst[TIMER_INDEX_10NS])               /**< \brief time constant equal to 10ns */
#define TimeConst_100ns (Ifx_TimeConst[TIMER_INDEX_100NS])              /**< \brief time constant equal to 100ns */
#define TimeConst_1us   (Ifx_TimeConst[TIMER_INDEX_1US])                /**< \brief time constant equal to 1us */
#define TimeConst_10us  (Ifx_TimeConst[TIMER_INDEX_10US])               /**< \brief time constant equal to 10us */
#define TimeConst_100us (Ifx_TimeConst[TIMER_INDEX_100US])              /**< \brief time constant equal to 100us */
#define TimeConst_1ms   (Ifx_TimeConst[TIMER_INDEX_1MS])                /**< \brief time constant equal to 1ms */
#define TimeConst_10ms  (Ifx_TimeConst[TIMER_INDEX_10MS])               /**< \brief time constant equal to 10ms */
#define TimeConst_100ms (Ifx_TimeConst[TIMER_INDEX_100MS])              /**< \brief time constant equal to 100ms */
#define TimeConst_1s    (Ifx_TimeConst[TIMER_INDEX_1S])                 /**< \brief time constant equal to 1s */
#define TimeConst_10s   (Ifx_TimeConst[TIMER_INDEX_10S])                /**< \brief time constant equal to 10s */
#define TimeConst_100s  (Ifx_TimeConst[TIMER_INDEX_100S])               /**< \brief time constant equal to 100s */
...
IFX_INLINE Ifx_TickTime Ifx_now(void)
{
    ...
    interruptState = Ifx_disableInterrupts();
...
#if !defined(IFX_ILLD_PPU_USAGE)
    Ifx_restoreInterrupts(interruptState);
#endif
    return stmNow;
}
...
IFX_INLINE boolean Ifx_poll(volatile boolean *test, Ifx_TickTime timeout)
{
    ...
    while ((*test == FALSE) && (Ifx_isDeadLine(deadLine) == FALSE))
    {}
...
IFX_INLINE Ifx_TickTime Ifx_timingNoInterruptEnd(Ifx_TickTime since, boolean interruptEnabled)
{
    ...
    stmNow = Ifx_nowWithoutCriticalSection();
    Ifx_restoreInterrupts(interruptEnabled);
...
IFX_INLINE Ifx_TickTime Ifx_timingNoInterruptStart(boolean *interruptEnabled)
{
    *interruptEnabled = Ifx_disableInterrupts();

    return Ifx_nowWithoutCriticalSection();
}
#endif
...
IFX_INLINE void Ifx_wait(Ifx_TickTime timeout)
{
    ...
    while (Ifx_isDeadLine(deadLine) == FALSE)
    {}
}
