...
boolean IfxGpt12_IncrEnc_init(IfxGpt12_IncrEnc *driver, const IfxGpt12_IncrEnc_Config *config)
{
    ...
    /* Gets the GPT12 module base address */
...
#if (IFXGPT12_TIMER_T3_MODE_EDGE_DETECTION)
...
#endif /* #if (IFXGPT12_TIMER_T3_MODE_EDGE_DETECTION) */
...
#if (IFXGPT12_TIMER_INTERRUPT_DISABLE)
        t2Con.B.T2IRDIS = 1u;
#endif /* #if (IFXGPT12_TIMER_INTERRUPT_DISABLE) */
...
IFX_STATIC void IfxGpt12_IncrEnc_updateRawPosition(IfxGpt12_IncrEnc *driver)
{
    ...
    boolean t3ModeIncremental = (gpt12->T3CON.B.T3M == IfxGpt12_Mode_incrementalInterfaceEdgeDetection) ||
                                (gpt12->T3CON.B.T3M == IfxGpt12_Mode_incrementalInterfaceRotationDetection);
...
IFX_STATIC void IfxGpt12_IncrEnc_updateFromT2(IfxGpt12_IncrEnc *driver)
{
    Ifx_GPT12 *gpt12 = driver->module;
    sint32     newPosition;
    /* Determines the direction based on the T2RDIR bit */
    ...
    /* Updates the speed based on the new position */
...
IFX_STATIC void IfxGpt12_IncrEnc_updateFromT3(IfxGpt12_IncrEnc *driver)
{
    Ifx_GPT12 *gpt12 = driver->module;
    sint32     newPosition;
    /* Determines the direction based on the T3RDIR bit */
...
IFX_STATIC void IfxGpt12_IncrEnc_updateSpeed(IfxGpt12_IncrEnc *driver, sint32 newPosition)
{
    ...
    if (driver->direction == IfxGpt12_IncrEnc_Direction_forward)
    {
    	/* Calculates the position difference for forward direction */
        diff = newPosition - driver->rawPosition;
    }
    else
    {
    	/* Calculates the position difference for backward direction */
    ...
    /* Calculates the speed using the position difference and the speed constant */
    ...
    /* Updates the driver's speed with the calculated value */
    driver->speed = speed;
}
...
IFX_STATIC void IfxGpt12_IncrEnc_updateSpeedFromT3(IfxGpt12_IncrEnc *driver, sint32 newPosition)
{
    ...
    /* Checks if the difference exceeds the speed mode threshold */
    ...
    else
    {
        ...
        if (srcT5->B.SRR != 1)
        {
            ...
            /* Checks if a new value is captured */
            if (srcCap->B.SRR != 0)
            {
...
IFX_STATIC void IfxGpt12_IncrEnc_updateFromT4(IfxGpt12_IncrEnc *driver)
{
    Ifx_GPT12 *gpt12 = driver->module;
    sint32     newPosition;
    /* Determines the direction based on the T4RDIR bit */
