/**
 * \file IfxGtm_Tim.h
 * \brief GTM  basic functionality
 * \ingroup IfxLld_Gtm
 *
 * \version iLLD-TC4-v2.6.0
 * \copyright Copyright (c) 2026 Infineon Technologies AG. All rights reserved.
 *
 *
 *
 *                                 IMPORTANT NOTICE
 *
 * Infineon Technologies AG (Infineon) licenses this file to you under the
 * Infineon Automotive SW Lab License v2025-01 (IFASLL). You may not use
 * this file except in compliance with IFASLL.
 *
 * The full license text is contained in IFASLL202501.pdf delivered with this SW.
 * Unless required by applicable law or agreed to in writing, software distributed
 * under this license is distributed "AS IS" without any warranty or liability of any
 * kind and Infineon hereby expressly disclaims any warranties or representations,
 * whether express, implied, statutory or otherwise, including but not limited to
 * warranties of workmanship, merchantability, fitness for a particular purpose,
 * defects in the licensed items, or non-infringement of third parties'
 * intellectual property rights. See the full license text for the specific
 * language governing permissions and limitations under IFASLL.
 *
 *
 *
 *
 * \defgroup IfxLld_Gtm_Std_Tim Tim Basic Functionality
 * \ingroup IfxLld_Gtm_Std
 * \defgroup IfxLld_Gtm_Std_Tim_Enumerations TIM Enumerations
 * \ingroup IfxLld_Gtm_Std_Tim
 * \defgroup IfxLld_Gtm_Std_Tim_Channel_Functions TIM Channel Functions
 * \ingroup IfxLld_Gtm_Std_Tim
 */

#ifndef IFXGTM_TIM_H
#define IFXGTM_TIM_H 1

/******************************************************************************/
/*----------------------------------Includes----------------------------------*/
/******************************************************************************/

#include "_Impl/IfxGtm_cfg.h"
#if !defined(IFX_ILLD_PPU_USAGE)
#include "Cpu/Std/IfxCpu_Intrinsics.h"
#endif
#if defined(IFX_ILLD_PPU_USAGE)
#include "Ppu/Std/IfxPpu_Intrinsics.h"
#endif
#include "IfxGtm_bf.h"
#include "Src/Std/IfxSrc.h"
#include "IfxGtm.h"
#include "IfxGtm_Cmu.h"

/******************************************************************************/
/*-----------------------------------Macros-----------------------------------*/
/******************************************************************************/

/** \brief TIMINSEL.CHxSEL value to select ADC boundary flag input path for TIM channel */
#define IFX_GTM_TIMINSEL_CHSEL_ADC_BOUNDARY_FLAG (0xCu)

/******************************************************************************/
/*--------------------------------Enumerations--------------------------------*/
/******************************************************************************/

/** \addtogroup IfxLld_Gtm_Std_Tim_Enumerations
 * \{ */
/** \brief Enum for selection for CNTS register.
 * Definition in Ifx_GTM_TIM_CH_CTRLB.CNTS_SEL
 */
typedef enum
{
    IfxGtm_Tim_CntsSel_cntReg = 0,
    IfxGtm_Tim_CntsSel_tbuTs0
} IfxGtm_Tim_CntsSel;

/** \brief Enum for Filter counter.
 * Definition in Ifx_GTM_TIM_CH_ECTRL.B.EFLT_CTR_RE and Ifx_GTM_TIM_CH_ECTRL.B.EFLT_CTR_FE
 */
typedef enum
{
    IfxGtm_Tim_FilterCounter_upDown = 0,
    IfxGtm_Tim_FilterCounter_hold
} IfxGtm_Tim_FilterCounter;

/** \brief Enum for Filter counter Frequency Selection.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.FLT_CNT_FRQ
 */
typedef enum
{
    IfxGtm_Tim_FilterCounterFreqSel_cmuClk0 = 0,  /**< \brief FLT_CNT counts with CMU_CLK0 */
    IfxGtm_Tim_FilterCounterFreqSel_cmuClk1 = 1,  /**< \brief FLT_CNT counts with CMU_CLK1 */
    IfxGtm_Tim_FilterCounterFreqSel_cmuClk6 = 2,  /**< \brief FLT_CNT counts with CMU_CLK6 */
    IfxGtm_Tim_FilterCounterFreqSel_cmuClk7 = 3   /**< \brief FLT_CNT counts with CMU_CLK7 */
} IfxGtm_Tim_FilterCounterFreqSel;

/** \brief Enum for Filter mode.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.FLT_MODE_RE and Ifx_GTM_TIM_CH_CTRL.B.FLT_MODE_FE
 */
typedef enum
{
    IfxGtm_Tim_FilterMode_immediateEdgePropagation = 0,  /**< \brief Immediate edge Propagation mode */
    IfxGtm_Tim_FilterMode_individualDeglitchTime         /**< \brief Individual deglitch mode */
} IfxGtm_Tim_FilterMode;

/** \brief Enum for GPR0/GPR1 register selection.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.GPR0_SEL and Ifx_GTM_TIM_CH_CTRL.B.GPR1_SEL
 */
typedef enum
{
    IfxGtm_Tim_GprSel_tbuTs0 = 0,  /**< \brief Use TBU_TS0 as input */
    IfxGtm_Tim_GprSel_tbuTs1,      /**< \brief Use TBU_TS1 as input */
    IfxGtm_Tim_GprSel_tbuTs2,      /**< \brief Use TBU_TS2 as input */
    IfxGtm_Tim_GprSel_cnts         /**< \brief Use CNTS as input */
} IfxGtm_Tim_GprSel;

/** \brief Enum for Channel Input Control.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.CICTRL
 */
typedef enum
{
    IfxGtm_Tim_Input_currentChannel = 0,  /**< \brief Use signal TIM_IN(x) as input for channel x */
    IfxGtm_Tim_Input_adjacentChannel      /**< \brief Use signal TIM_IN(x-1) as input for channel x (or TIM_IN(m-1) if x is 0) */
} IfxGtm_Tim_Input;

/** \brief Enum for Irq mode.
 */
typedef enum
{
    IfxGtm_Tim_IrqMode_level = 0,        /**< \brief Level Mode */
    IfxGtm_Tim_IrqMode_pulse,            /**< \brief Pulse Mode */
    IfxGtm_Tim_IrqMode_pulseNotify,      /**< \brief pulse notify Mode */
    IfxGtm_Tim_IrqMode_singlePulseMode,  /**< \brief Single Pulse Mode */
    IfxGtm_Tim_IrqMode_none = -1         /**< \brief none */
} IfxGtm_Tim_IrqMode;

/** \brief Enum for Irq type.
 */
typedef enum
{
    IfxGtm_Tim_IrqType_newVal        = 0,  /**< \brief New measurement value detected by SMU of channel */
    IfxGtm_Tim_IrqType_ecntOverflow  = 1,  /**< \brief ECNT counter overflow of channel */
    IfxGtm_Tim_IrqType_cntOverflow   = 2,  /**< \brief CNT counter overflow of channel */
    IfxGtm_Tim_IrqType_gprOverflow   = 3,  /**< \brief GPR0 and GPR1 data overflow */
    IfxGtm_Tim_IrqType_timeoutDetect = 4,  /**< \brief Time out reached for input signal of channel */
    IfxGtm_Tim_IrqType_glitchDetect  = 5   /**< \brief A glitch was detected by the TIM filter of channel */
} IfxGtm_Tim_IrqType;

/** \brief Enum for TIM channel mode.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.TIM_MODE
 */
typedef enum
{
    IfxGtm_Tim_Mode_pwmMeasurement = 0,  /**< \brief TPWM */
    IfxGtm_Tim_Mode_pulseIntegration,    /**< \brief TPIM */
    IfxGtm_Tim_Mode_inputEvent,          /**< \brief TIEM */
    IfxGtm_Tim_Mode_inputPrescaler,      /**< \brief TIPM */
    IfxGtm_Tim_Mode_bitCompression,      /**< \brief TBCM */
    IfxGtm_Tim_Mode_gatedPeriodic,       /**< \brief TGPS */
    IfxGtm_Tim_Mode_serialShift          /**< \brief TSSM */
} IfxGtm_Tim_Mode;

/** \brief Enum for Timeout control.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.TOCTRL
 */
typedef enum
{
    IfxGtm_Tim_Timeout_disabled,     /**< \brief Timeout feature disabled */
    IfxGtm_Tim_Timeout_risingEdge,   /**< \brief Timeout feature enabled for rising edge only */
    IfxGtm_Tim_Timeout_fallingEdge,  /**< \brief Timeout feature enabled for falling edge only */
    IfxGtm_Tim_Timeout_bothEdge      /**< \brief Timeout feature enabled for both edges */
} IfxGtm_Tim_Timeout;

/** \} */

/******************************************************************************/
/*-----------------------------Data Structures--------------------------------*/
/******************************************************************************/

/** \brief Channel control
 */
typedef struct
{
    boolean         enable;                                             /**< \brief TIM channel enable */
    IfxGtm_Tim_Mode mode;                                               /**< \brief TIM channel Mode */
    boolean         enableOneShotMode;                                  /**< \brief enable OneShot Mode (disable - continuous operation mode) */
    boolean         enableAruRouting;                                   /**< \brief GPR0 and GPR1 register values routed to ARU.
                                                                         * 0- Registers content not routed.
                                                                         * 1- Registers content routed */
    boolean            channelInputControl;                             /**< \brief 0- use signal TIM_IN(x) as input for channel .
                                                                         * 1- use signal TIM_IN(x-1) as input for channel x (or TIM_IN(m-1) if x is 0) */
    IfxGtm_Tim_GprSel  gpr0Sel;                                         /**< \brief Selection for GPR0 register */
    IfxGtm_Tim_GprSel  gpr1Sel;                                         /**< \brief Selection for GPR1 register */
    IfxGtm_Tim_CntsSel cntsSel;                                         /**< \brief Selection for CNTS register */
    boolean            signalLevelControl;                              /**< \brief Signal level control
                                                                         * 0- Measurement starts with falling edge (low level measurement).
                                                                         * 1- Measurement starts with rising edge (high level measurement)
                                                                         * In TIM_MODE=0b110 (TSSM) the bit field DSL defines the shift direction.
                                                                         * 0- Shift left
                                                                         * 1- Shift right */
    boolean ignoreSignalLevel;                                          /**< \brief ignore Signal Level.
                                                                         * 0- use DSL bit for selecting active signal level (TIEM).
                                                                         * 1- ignore DSL and treat both edges as active edge (TIEM). */
    boolean enableCounterReset;                                         /**< \brief Enables resetting of counter in certain modes ( TIM_MODE=0b101 (TGPS) / TIM_MODE=0b000 (TPWM))
                                                                         * 0- ECNT counter operating in wrap around mode / ECNT counter operating in wrap around mode, CNT is reset on active input edge defined by DSL
                                                                         * 1 - ECNT counter is reset with periodic sampling / ECNT counter operating in wrap around mode, CNT is reset on active and inactive input edge else ECNT counter operating in wrap around mode;
                                                                         * In TIM_MODE=0b110 (TSSM) the bit field ECNT_RESET defines the initial polarity for the shift register. */
    boolean                         enableFilter;                       /**< \brief enable Filter */
    IfxGtm_Tim_FilterCounterFreqSel filterCounterFrequency;             /**< \brief Filter counter frequency select */
    boolean                         enableExternalCaptureMode;          /**< \brief Enables external capture mode */
    IfxGtm_Tim_FilterMode           filterModeRisingEdge;               /**< \brief Filter mode for rising edge */
    IfxGtm_Tim_FilterCounter        filterCounterModeRisingEdge;        /**< \brief Filter counter mode for rising edge */
    IfxGtm_Tim_FilterMode           filterModeFallingEdge;              /**< \brief Filter mode for falling edge */
    IfxGtm_Tim_FilterCounter        filterCounterModeFallingEdge;       /**< \brief Filter counter mode for falling edge */
    IfxGtm_Cmu_Clk                  clkSel;                             /**< \brief CMU clock source select for channel */
    boolean                         extendEdgeCounterOverflow;          /**< \brief Extended Edge counter overflow behavior
                                                                         * 0- Overflow will be signaled on ECNT bit width = 8
                                                                         * 1- Overflow will be signaled on EECNT bit width (full range) */
    boolean            egpr0Sel;                                        /**< \brief Extension of GPR0_SEL bit field */
    boolean            egpr1Sel;                                        /**< \brief Extension of GPR1_SEL bit field */
    IfxGtm_Tim_Timeout timeoutControl;                                  /**< \brief Timeout control */
} IfxGtm_Tim_ChannelControl;

typedef struct
{
    uint8 mode;        /**< \brief Input source to Channel. multi-core encoding in use (MODE_x(1) defines the state of the signal)
                        * 00- State is 0 (ignore write access)
                        * 01- Change state to 0
                        * 10- Change state to 1
                        * 11- State is 1 (ignore write access) */
    uint8 value;       /**< \brief Value to be fed to Channel. Multicore encoding in use (VAL_x(1) defines the state of the signal).
						* 00- State is 0 (ignore write access)
						* 01- Change state to 0
						* 10- Change state to 1
						* 11- State is 1 (ignore write access) */
} IfxGtm_Tim_InputSourceSelect;

/** \addtogroup IfxLld_Gtm_Std_Tim_Channel_Functions
 * \{ */

/******************************************************************************/
/*-------------------------Inline Function Prototypes-------------------------*/
/******************************************************************************/

/** \brief This function clears the Aux input Source selection for the  given channel.
 *
 * \param[inout] gtm      Pointer to GTM module
 * \param[in]    clsIndex TIM instance number. Range: \ref IfxGtm_Cluster.
 * \param[in]    channel  Channel index. Range: \ref IfxGtm_Tim_Ch.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearAuxInputSourceSelection(Ifx_GTM *gtm, IfxGtm_Cluster clsIndex, IfxGtm_Tim_Ch channel);

/** \brief Clears the counter overflow flag.
 *
 * \param[inout] channel TIM channel pointer.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearCntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Clears the data lost flag.
 *
 * \param[inout] channel TIM channel pointer.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearDataLostEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Clears the event counter overflow flag.
 *
 * \param[inout] channel TIM channel pointer.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearEcntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Clears the glitch flag.
 *
 * \param[inout] channel TIM channel pointer.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearGlitchEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Clears the new value flag.
 *
 * \param[inout] channel TIM channel pointer.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearNewValueEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Clear the interrupt notification.
 *
 * \param[inout] channel TIM channel pointer.
 * \param[in]    irqType Interrupt type. Range: \ref IfxGtm_Tim_IrqType.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_clearNotification(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_IrqType irqType);

/** \brief Returns the Capture Clock Frequency.
 *
 * \param[in] gtm     Pointer to GTM module.
 * \param[in] channel Pointer to TIM channel base.
 *
 * \retval float32 Capture clock frequency.
 */
IFX_INLINE float32 IfxGtm_Tim_Ch_getCaptureClockFrequency(Ifx_GTM *gtm, Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Returns the capture clock source selected.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval IfxGtm_Cmu_Clk Capture clock source. Range: \ref IfxGtm_Cmu_Clk.
 */
IFX_INLINE IfxGtm_Cmu_Clk IfxGtm_Tim_Ch_getCaptureClockSource(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Returns the Filter clock source selected.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval IfxGtm_Cmu_Tim_Filter_Clk Filter clock source. Range: \ref IfxGtm_Cmu_Tim_Filter_Clk.
 */
IFX_INLINE IfxGtm_Cmu_Tim_Filter_Clk IfxGtm_Tim_Ch_getFilterClockSource(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Returns the status of channel  notification.
 *
 * \param[in] channel TIM channel pointer.
 * \param[in] irqType Interrupt type. Range: \ref IfxGtm_Tim_IrqType.
 *
 * \retval Status of channel notification.
 */
IFX_INLINE boolean IfxGtm_Tim_Ch_getNotification(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_IrqType irqType);

/** \brief Returns a pointer to the TIM channel SRC.
 *
 * \param[in] gtm Pointer to GTM module.
 * \param[in] tim Specifies the tim module no. Range: \ref IfxGtm_Tim.
 * \param[in] channel Channel index. Range: \ref IfxGtm_Tim_Ch.
 *
 * \retval volatile Ifx_SRC_SRCR* Pointer to the TIM channel SRC.
 */
IFX_INLINE volatile Ifx_SRC_SRCR *IfxGtm_Tim_Ch_getSrcPointer(Ifx_GTM *gtm, IfxGtm_Tim tim, IfxGtm_Tim_Ch channel);

/** \brief Returns the Timeout Clock Frequency.
 *
 * \param[in] gtm     Pointer to GTM module.
 * \param[in] channel Pointer to TIM channel base.
 *
 * \retval float32 Timeout clock frequency.
 */
IFX_INLINE float32 IfxGtm_Tim_Ch_getTimeoutClockFrequency(Ifx_GTM *gtm, Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Returns the Timeout clock source selected.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval IfxGtm_Cmu_Clk Timeout clock source. Range: \ref IfxGtm_Cmu_Clk.
 */
IFX_INLINE IfxGtm_Cmu_Clk IfxGtm_Tim_Ch_getTimeoutClockSource(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Test the counter overflow flag.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval TRUE if the flag is set, else FALSE.
 */
IFX_INLINE boolean IfxGtm_Tim_Ch_isCntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Test the data lost flag.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval TRUE if the flag is set, else FALSE.
 */
IFX_INLINE boolean IfxGtm_Tim_Ch_isDataLostEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Test the event counter overflow flag.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval TRUE if the flag is set, else FALSE.
 */
IFX_INLINE boolean IfxGtm_Tim_Ch_isEcntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Test the glitch flag.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval TRUE if the flag is set, else FALSE.
 */
IFX_INLINE boolean IfxGtm_Tim_Ch_isGlitchEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Test the new value flag.
 *
 * \param[in] channel TIM channel pointer.
 *
 * \retval TRUE if the flag is set, else FALSE.
 */
IFX_INLINE boolean IfxGtm_Tim_Ch_isNewValueEvent(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief set the Auxiliary input source selection of a particular channel.
 *
 * \param[inout] gtm      Pointer to GTM module.
 * \param[in]    clsIndex TIM instance number. Range: \ref IfxGtm_Cluster.
 * \param[in]    channel  Channel index. Range: \ref IfxGtm_Tim_Ch.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setAuxInputSourceSelection(Ifx_GTM *gtm, IfxGtm_Cluster clsIndex, IfxGtm_Tim_Ch channel);

/** \brief Set the channel notification.
 *
 * \param[inout] channel           TIM channel pointer.
 * \param[in]    irqOnNewVal       If TRUE, the interrupt on new value is enabled.
 * \param[in]    irqOnCntOverflow  If TRUE, the interrupt on CNT overflow is enabled.
 * \param[in]    irqOnEcntOverflow If TRUE, the interrupt on ECNT overflow is enabled.
 * \param[in]    irqOnDatalost     If TRUE, the interrupt on data lost (GPR0, GPR1) is enabled.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setChannelNotification(Ifx_GTM_CLS_TIM_CH *channel, boolean irqOnNewVal, boolean irqOnCntOverflow, boolean irqOnEcntOverflow, boolean irqOnDatalost);

/** \brief Set the channel clock source.
 *
 * \param[inout] channel TIM channel pointer.
 * \param[in]    clk     Selected clock. Range: \ref IfxGtm_Cmu_Clk.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setClockSource(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Cmu_Clk clk);

/** \brief This function configures the Edge counter register.
 *
 * \param[inout] channel     TIM channel pointer.
 * \param[in]    edgeCounter Edge counter value. Range: 0 to 0xFF.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setEdgeCounter(Ifx_GTM_CLS_TIM_CH *channel, uint8 edgeCounter);

/** \brief This function configures the Falling edge Filter Parameter.
 *
 * \param[inout] channel    TIM channel pointer.
 * \param[in]    filterTime Falling edge Filter Parameter. Range: 0 to 0xFFFFFF.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setFallingEdgeFilterTime(Ifx_GTM_CLS_TIM_CH *channel, uint32 filterTime);

/** \brief Set the filter notification.
 *
 * \param[inout] channel     TIM channel pointer.
 * \param[in]    irqOnGlitch If TRUE, the interrupt on glitch is enabled.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setFilterNotification(Ifx_GTM_CLS_TIM_CH *channel, boolean irqOnGlitch);

/** \brief This function configures the Input source selection register.
 *
 * \param[inout] tim         TIM instance number.
 * \param[in]    channel     Channel index. Range: \ref IfxGtm_Tim_Ch.
 * \param[in]    inputSource Input source to Channel.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setInputSourceSelection(Ifx_GTM_CLS_TIM *tim, IfxGtm_Tim_Ch channel, IfxGtm_Tim_InputSourceSelect inputSource);

/** \brief This function configures the TIM channel IRQ mode.
 *
 * \param[inout] channel TIM channel pointer.
 * \param[in]    mode    Notification Mode. Range: \ref IfxGtm_IrqMode.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setNotificationMode(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_IrqMode mode);

/** \brief Set the Interrupt Notification.
 *
 * \param[inout] channel TIM channel pointer.
 * \param[in]    irqType Interrupt type. Range: \ref IfxGtm_Tim_IrqType.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setNotification(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_IrqType irqType);

/** \brief Set the timeout notification.
 *
 * \param[inout] channel      TIM channel pointer.
 * \param[in]    irqOnTimeout If TRUE, the interrupt on timeout is enabled.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setTimeoutNotification(Ifx_GTM_CLS_TIM_CH *channel, boolean irqOnTimeout);

/** \brief This function configures theRaising edge Filter Parameter.
 *
 * \param[inout] channel    TIM channel pointer.
 * \param[in]    filterTime Raising edge Filter Parameter. Range: 0 to 0xFFFFFF.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setRisingEdgeFilterTime(Ifx_GTM_CLS_TIM_CH *channel, uint32 filterTime);

/** \brief This function configures the Shadow counter register.
 *
 * \param[inout] channel       TIM channel pointer.
 * \param[in]    shadowCounter Shadow counter value. Range: 0 to 0xFFFFFF.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setShadowCounter(Ifx_GTM_CLS_TIM_CH *channel, uint32 shadowCounter);

/** \brief Returns the pointer to the TIM channel.
 *
 * \param[in] tim     Pointer to Tim base.
 * \param[in] channel TIM channel. Range: \ref IfxGtm_Tim_Ch.
 *
 * \retval volatile Ifx_GTM_CLS_TIM_CH* Pointer to TIM channel base.
 */
IFX_INLINE volatile Ifx_GTM_CLS_TIM_CH *IfxGtm_Tim_getChannel(Ifx_GTM_CLS_TIM *tim, IfxGtm_Tim_Ch channel);

/** \brief Reset the specified TIM channel.
 *
 * \param[inout] tim     Pointer to GTM TIM instance.
 * \param[in]    channel Channel index. Range: \ref IfxGtm_Tim_Ch.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_resetChannel(Ifx_GTM_CLS_TIM *tim, IfxGtm_Tim_Ch channel);

/******************************************************************************/
/*-------------------------Global Function Prototypes-------------------------*/
/******************************************************************************/

/** \brief Returns the Filter Clock Frequency.
 *
 * \param[in] gtm     Pointer to GTM module.
 * \param[in] channel Pointer to TIM channel base.
 *
 * \retval float32 Filter clock frequency.
 */
IFX_EXTERN float32 IfxGtm_Tim_Ch_getFilterClockFrequency(Ifx_GTM *gtm, Ifx_GTM_CLS_TIM_CH *channel);

/** \brief This function configures the TIM channel Control register.
 *
 * \param[inout] channel TIM channel pointer.
 * \param[in]    control channel control configuration.
 *
 * \retval None
 */
IFX_EXTERN void IfxGtm_Tim_Ch_setControl(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_ChannelControl control);

/** \} */

/******************************************************************************/
/*-------------------------Inline Function Prototypes-------------------------*/
/******************************************************************************/

/** \brief sets the TIMINSEL register with the required mux setting.
 *
 * \param[in] timIndex     Index of TIM module. Range: \ref IfxGtm_Tim.
 * \param[in] channelIndex Index of TIM channel. Range: \ref IfxGtm_Tim_Ch.
 * \param[in] tinSel       MUX value selection. Range: 0 to 0xFFFFFFFF.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_Ch_setTimTin(IfxGtm_Tim timIndex, IfxGtm_Tim_Ch channelIndex, uint32 tinSel);

/** \brief Connect TIM channel input to ADC boundary-flag path (Output from ADC Multiplexer).
 *
 * TIM channels are hardwired to the ADC Boundary Flag outputs as follows:
 * - TIMx.CH0, TIMx.CH1 -> OUTSEL0
 * - TIMx.CH2, TIMx.CH3 -> OUTSEL1
 * - TIMx.CH4, TIMx.CH5 -> OUTSEL2
 * - TIMx.CH6, TIMx.CH7 -> OUTSEL3
 *
 * \param[in] cluster    TIM cluster index. Range: \ref IfxGtm_Cluster
 * \param[in] timChannel TIM channel index within the cluster. Range: \ref IfxGtm_Tim_Ch
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_selectAdcBoundaryFlag(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel);

/** \brief Get count of TIM channel.
 *
 * \param[in] channel Pointer to TIM channel base.
 *
 * \retval uint32 Number of count. Range: 0 to 0xFFFFFF.
 */
IFX_INLINE uint32 IfxGtm_Tim_Ch_getCountValue(Ifx_GTM_CLS_TIM_CH *channel);

/** \brief Get shadow count of TIM channel.
 *
 * \param[in] channel Pointer to TIM channel base.
 *
 * \retval uint32 Number of shadow count. Range: 0 to 0xFFFFFF.
 */
IFX_INLINE uint32 IfxGtm_Tim_Ch_getShadowCountValue(Ifx_GTM_CLS_TIM_CH *channel);

/* This API fetches the output status of the filter signal of a given TIM input channel.
 *
 * \param[in] cluster Cluster of GTM which contains the trigger source Range: \ref: IfxGtm_Cluster
 * \param[in] channel Channel index. Range: \ref: IfxGtm_Tim_Ch
 *
 * \retval TRUE If the output filter source signal is high
 *         FALSE If the output filter source signal is low
 */
IFX_INLINE boolean IfxGtm_Tim_isFilterOutSignalHigh(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel);

/* This API fetches the status of its filter input source of a given TIM input channel.
 * Filter input is possible pre-processed signal by LUT block. The result provides the current status, at immediate possible point in time when the API is called.
 *
 * \param[in] cluster Cluster of GTM which contains the trigger source Range: \ref: IfxGtm_Cluster
 * \param[in] channel Channel index. Range: \ref: IfxGtm_Tim_Ch
 *
 * \retval TRUE If the input filter source signal is high
 *         FALSE If the input filter source signal is low
 */
IFX_INLINE boolean IfxGtm_Tim_isFilterInputSourceSignalHigh(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel);

/* This API fetches the status of its own input source of a given TIM input channel.
 * In this case the result provides the current status, at immediate possible point in time when the API is called.
 *
 * \param[in] cluster Cluster of GTM which contains the trigger source Range: \ref: IfxGtm_Cluster
 * \param[in] channel Channel index. Range: \ref: IfxGtm_Tim_Ch
 *
 * \retval TRUE If the input signal status is high
 *         FALSE If the input signal status is low
 */
IFX_INLINE boolean IfxGtm_Tim_isInputSignalHigh(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel);

/******************************************************************************/
/*---------------------Inline Function Implementations------------------------*/
/******************************************************************************/

IFX_INLINE void IfxGtm_Tim_Ch_clearAuxInputSourceSelection(Ifx_GTM *gtm, IfxGtm_Cluster clsIndex, IfxGtm_Tim_Ch channel)
{
    Ifx_GTM_CLS_CCM_TIM_AUX_IN_SRC src;
    src.U                                   = gtm->CLS[clsIndex].CCM.TIM_AUX_IN_SRC.U;
    src.U                                  &= (uint32)(~((uint32)1u << (uint8)channel));
    gtm->CLS[clsIndex].CCM.TIM_AUX_IN_SRC.U = src.U;
}


IFX_INLINE void IfxGtm_Tim_Ch_clearCntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    channel->IRQ_NOTIFY.U = 0x1u << IFX_GTM_CLS_TIM_CH_IRQ_NOTIFY_CNTOFL_OFF;
}


IFX_INLINE void IfxGtm_Tim_Ch_clearDataLostEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    channel->IRQ_NOTIFY.U = 0x1u << IFX_GTM_CLS_TIM_CH_IRQ_NOTIFY_GPROFL_OFF;
}


IFX_INLINE void IfxGtm_Tim_Ch_clearEcntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    channel->IRQ_NOTIFY.U = 0x1u << IFX_GTM_CLS_TIM_CH_IRQ_NOTIFY_ECNTOFL_OFF;
}


IFX_INLINE void IfxGtm_Tim_Ch_clearGlitchEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    channel->IRQ_NOTIFY.U = 0x1u << IFX_GTM_CLS_TIM_CH_IRQ_NOTIFY_GLITCHDET_OFF;
}


IFX_INLINE void IfxGtm_Tim_Ch_clearNewValueEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    channel->IRQ_NOTIFY.U = 0x1u << IFX_GTM_CLS_TIM_CH_IRQ_NOTIFY_NEWVAL_OFF;
}


IFX_INLINE void IfxGtm_Tim_Ch_clearNotification(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_IrqType irqType)
{
    channel->IRQ_EN.U    &= (uint32)(~((uint32)1u << (uint8)irqType));
    channel->IRQ_NOTIFY.U = (uint32)((uint32)1u << (uint8)irqType);
}


IFX_INLINE float32 IfxGtm_Tim_Ch_getCaptureClockFrequency(Ifx_GTM *gtm, Ifx_GTM_CLS_TIM_CH *channel)
{
    IfxGtm_Cmu_Clk clk;

    clk = IfxGtm_Tim_Ch_getCaptureClockSource(channel);

    return IfxGtm_Cmu_getClkFrequency(gtm, clk, TRUE);
}


IFX_INLINE IfxGtm_Cmu_Clk IfxGtm_Tim_Ch_getCaptureClockSource(Ifx_GTM_CLS_TIM_CH *channel)
{
    return (IfxGtm_Cmu_Clk)channel->CTRL.B.CLK_SEL;
}


IFX_INLINE IfxGtm_Cmu_Tim_Filter_Clk IfxGtm_Tim_Ch_getFilterClockSource(Ifx_GTM_CLS_TIM_CH *channel)
{
    return (IfxGtm_Cmu_Tim_Filter_Clk)channel->CTRL.B.FLT_CNT_FRQ;
}


IFX_INLINE boolean IfxGtm_Tim_Ch_getNotification(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_IrqType irqType)
{
    boolean result;
    result = (boolean)((channel->IRQ_NOTIFY.U >> (uint8)irqType) & 0x1u);
    return result;
}


IFX_INLINE volatile Ifx_SRC_SRCR *IfxGtm_Tim_Ch_getSrcPointer(Ifx_GTM *gtm, IfxGtm_Tim tim, IfxGtm_Tim_Ch channel)
{
    IFX_UNUSED_PARAMETER(gtm)
    return &MODULE_SRC.GTM.TIM[tim].SR[channel];
}


IFX_INLINE float32 IfxGtm_Tim_Ch_getTimeoutClockFrequency(Ifx_GTM *gtm, Ifx_GTM_CLS_TIM_CH *channel)
{
    IfxGtm_Cmu_Clk clk;

    clk = IfxGtm_Tim_Ch_getTimeoutClockSource(channel);

    return IfxGtm_Cmu_getClkFrequency(gtm, clk, TRUE);
}


IFX_INLINE IfxGtm_Cmu_Clk IfxGtm_Tim_Ch_getTimeoutClockSource(Ifx_GTM_CLS_TIM_CH *channel)
{
    return (IfxGtm_Cmu_Clk)channel->TDUV.B.TCS;
}


IFX_INLINE boolean IfxGtm_Tim_Ch_isCntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    boolean event;
    event = (channel->IRQ_NOTIFY.B.CNTOFL == (uint8)1u) ? TRUE : FALSE;
    return event;
}


IFX_INLINE boolean IfxGtm_Tim_Ch_isDataLostEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    boolean event;
    event = (channel->IRQ_NOTIFY.B.GPROFL == (uint8)1u) ? TRUE : FALSE;
    return event;
}


IFX_INLINE boolean IfxGtm_Tim_Ch_isEcntOverflowEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    boolean event;
    event = (channel->IRQ_NOTIFY.B.ECNTOFL == (uint8)1u) ? TRUE : FALSE;
    return event;
}


IFX_INLINE boolean IfxGtm_Tim_Ch_isGlitchEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    boolean event;
    event = (channel->IRQ_NOTIFY.B.GLITCHDET == (uint8)1u) ? TRUE : FALSE;
    return event;
}


IFX_INLINE boolean IfxGtm_Tim_Ch_isNewValueEvent(Ifx_GTM_CLS_TIM_CH *channel)
{
    boolean event;
    event = (channel->IRQ_NOTIFY.B.NEWVAL == (uint8)1u) ? TRUE : FALSE;
    return event;
}


IFX_INLINE void IfxGtm_Tim_Ch_setAuxInputSourceSelection(Ifx_GTM *gtm, IfxGtm_Cluster clsIndex, IfxGtm_Tim_Ch channel)
{
    Ifx_GTM_CLS_CCM_TIM_AUX_IN_SRC src;
    src.U                                   = gtm->CLS[clsIndex].CCM.TIM_AUX_IN_SRC.U;
    src.U                                  |= (uint32)((uint32)1u << (uint8)channel);
    gtm->CLS[clsIndex].CCM.TIM_AUX_IN_SRC.U = src.U;
}


IFX_INLINE void IfxGtm_Tim_Ch_setChannelNotification(Ifx_GTM_CLS_TIM_CH *channel, boolean irqOnNewVal, boolean irqOnCntOverflow, boolean irqOnEcntOverflow, boolean irqOnDatalost)
{
    Ifx_GTM_CLS_TIM_CH_IRQ_EN en;
    en.U                = channel->IRQ_EN.U;

    en.B.NEWVAL_IRQ_EN  = (irqOnNewVal == TRUE) ? (uint8)1u : (uint8)0u;
    en.B.CNTOFL_IRQ_EN  = (irqOnCntOverflow == TRUE) ? (uint8)1u : (uint8)0u;
    en.B.ECNTOFL_IRQ_EN = (irqOnEcntOverflow == TRUE) ? (uint8)1u : (uint8)0u;
    en.B.GPROFL_IRQ_EN  = (irqOnDatalost == TRUE) ? (uint8)1u : (uint8)0u;

    channel->IRQ_EN.U   = en.U;
}


IFX_INLINE void IfxGtm_Tim_Ch_setClockSource(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Cmu_Clk clk)
{
    channel->CTRL.B.CLK_SEL = (uint8)clk;
}


IFX_INLINE void IfxGtm_Tim_Ch_setEdgeCounter(Ifx_GTM_CLS_TIM_CH *channel, uint8 edgeCounter)
{
    channel->CNTS.B.ECNT = edgeCounter;
}


IFX_INLINE void IfxGtm_Tim_Ch_setFallingEdgeFilterTime(Ifx_GTM_CLS_TIM_CH *channel, uint32 filterTime)
{
    channel->FLT_FE.B.FLT_FE = filterTime;
}


IFX_INLINE void IfxGtm_Tim_Ch_setFilterNotification(Ifx_GTM_CLS_TIM_CH *channel, boolean irqOnGlitch)
{
    Ifx_GTM_CLS_TIM_CH_IRQ_EN en;
    en.U                  = channel->IRQ_EN.U;
    en.B.GLITCHDET_IRQ_EN = (irqOnGlitch == TRUE) ? (uint8)1u : (uint8)0u;

    channel->IRQ_EN.U     = en.U;
}


IFX_INLINE void IfxGtm_Tim_Ch_setInputSourceSelection(Ifx_GTM_CLS_TIM *tim, IfxGtm_Tim_Ch channel, IfxGtm_Tim_InputSourceSelect inputSource)
{
    uint8 shift     = ((uint8)channel << 2u);

    uint8 setSource = inputSource.value;
    setSource    |= (inputSource.mode << 2u);

    tim->IN_SRC.U = (uint32)((uint32)setSource << shift);
}


IFX_INLINE void IfxGtm_Tim_Ch_setNotificationMode(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_IrqMode mode)
{
    Ifx_GTM_CLS_TIM_CH_IRQ_EN en;
    en.U = channel->IRQ_EN.U;

    /* Disable all interrupts of the interrupt set to change mode */
    channel->IRQ_EN.U            = 0u;
    channel->IRQ_MODE.B.IRQ_MODE = (uint8)mode;
    channel->IRQ_EN.U            = en.U; /* Set the values back */
}


IFX_INLINE void IfxGtm_Tim_Ch_setNotification(Ifx_GTM_CLS_TIM_CH *channel, IfxGtm_Tim_IrqType irqType)
{
    channel->IRQ_EN.U     |= (uint32)((uint32)1u << (uint8)irqType);
    channel->IRQ_NOTIFY.U |= (uint32)((uint32)1u << (uint8)irqType);
}


IFX_INLINE void IfxGtm_Tim_Ch_setTimeoutNotification(Ifx_GTM_CLS_TIM_CH *channel, boolean irqOnTimeout)
{
    Ifx_GTM_CLS_TIM_CH_IRQ_EN en;
    en.U              = channel->IRQ_EN.U;

    en.B.TODET_IRQ_EN = (irqOnTimeout == TRUE) ? (uint8)1u : (uint8)0u;

    channel->IRQ_EN.U = en.U;
}


IFX_INLINE void IfxGtm_Tim_Ch_setRisingEdgeFilterTime(Ifx_GTM_CLS_TIM_CH *channel, uint32 filterTime)
{
    channel->FLT_RE.B.FLT_RE = filterTime;
}


IFX_INLINE void IfxGtm_Tim_Ch_setShadowCounter(Ifx_GTM_CLS_TIM_CH *channel, uint32 shadowCounter)
{
    channel->CNTS.B.CNTS = shadowCounter;
}


IFX_INLINE volatile Ifx_GTM_CLS_TIM_CH *IfxGtm_Tim_getChannel(Ifx_GTM_CLS_TIM *tim, IfxGtm_Tim_Ch channel)
{
    return (volatile Ifx_GTM_CLS_TIM_CH *)(&tim->CH[channel].GPR0.U);
}


IFX_INLINE void IfxGtm_Tim_Ch_resetChannel(Ifx_GTM_CLS_TIM *tim, IfxGtm_Tim_Ch channel)
{
    tim->RST.U |= (uint32)1 << (uint32)channel;
}


IFX_INLINE void IfxGtm_Tim_Ch_setTimTin(IfxGtm_Tim timIndex, IfxGtm_Tim_Ch channelIndex, uint32 tinSel)
{
    uint32 shift = (uint32)channelIndex << 2u;

    Ifx__ldmst(&(MODULE_GTM.TIMINSEL[timIndex].U), ((uint32)0xFU << shift), (tinSel) << shift);
}


IFX_INLINE void IfxGtm_Tim_selectAdcBoundaryFlag(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel)
{
    uint32 shift = (uint32)timChannel * IFX_GTM_TIMINSEL_CH0SEL_LEN;
    uint32 value = (uint32)IFX_GTM_TIMINSEL_CHSEL_ADC_BOUNDARY_FLAG << shift;
    uint32 mask  = (uint32)IFX_GTM_TIMINSEL_CH0SEL_MSK << shift;

    Ifx__ldmst(&MODULE_GTM.TIMINSEL[cluster].U, mask, value);
}


IFX_INLINE uint32 IfxGtm_Tim_Ch_getCountValue(Ifx_GTM_CLS_TIM_CH *channel)
{
    return channel->CNT.B.CNT;
}


IFX_INLINE uint32 IfxGtm_Tim_Ch_getShadowCountValue(Ifx_GTM_CLS_TIM_CH *channel)
{
    return channel->CNTS.B.CNTS;
}


IFX_INLINE boolean IfxGtm_Tim_isFilterOutSignalHigh(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel)
{
	uint32 foutBitPos = (uint32)(1u << (timChannel + IFX_GTM_CLS_TIM_INP_VAL_F_OUT0_OFF));

    return ((MODULE_GTM.CLS[cluster].TIM.INP_VAL.U & foutBitPos) != 0u) ? TRUE : FALSE;
}


IFX_INLINE boolean IfxGtm_Tim_isFilterInputSourceSignalHigh(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel)
{
	uint32 finBitPos = (uint32)(1u << (timChannel + IFX_GTM_CLS_TIM_INP_VAL_F_IN0_OFF));

    return ((MODULE_GTM.CLS[cluster].TIM.INP_VAL.U & finBitPos) != 0u) ? TRUE : FALSE;
}


IFX_INLINE boolean IfxGtm_Tim_isInputSignalHigh(IfxGtm_Cluster cluster, IfxGtm_Tim_Ch timChannel)
{
	uint32 inBitPos = (uint32)(1u << (timChannel + IFX_GTM_CLS_TIM_INP_VAL_TIM_IN0_OFF));

	return ((MODULE_GTM.CLS[cluster].TIM.INP_VAL.U & inBitPos) != 0u) ? TRUE : FALSE;
}


#endif /* IFXGTM_TIM_H */
