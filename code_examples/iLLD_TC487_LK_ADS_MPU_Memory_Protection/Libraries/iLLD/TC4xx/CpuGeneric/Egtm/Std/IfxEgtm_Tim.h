...
#define IFX_EGTM_TIMINSEL_CHSEL_ADC_BOUNDARY_FLAG (0xCu)
 ...
 *
 * \param[in] cluster    TIM cluster index
 *                       Range: \ref: IfxEgtm_Cluster
 * \param[in] timChannel TIM channel index
 *                       Range: \ref: IfxEgtm_Tim_Ch
 *
 * \retval None
 */
IFX_INLINE void IfxEgtm_Tim_selectAdcBoundaryFlag(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel);
 ...
 *
 * \param[in] cluster Cluster of EGTM which contains the trigger source Range: \ref: IfxEgtm_Cluster
 * \param[in] channel Channel index. Range: \ref: IfxEgtm_Tim_Ch
 *
 * \retval TRUE If the output filter source signal is high
 *         FALSE If the output filter source signal is low
 */
IFX_INLINE boolean IfxEgtm_Tim_isFilterOutSignalHigh(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel);
 ...
 */
IFX_INLINE boolean IfxEgtm_Tim_isFilterInputSourceSignalHigh(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel);
 ...
 */
IFX_INLINE boolean IfxEgtm_Tim_isInputSignalHigh(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel);
...
IFX_INLINE void IfxEgtm_Tim_selectAdcBoundaryFlag(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel)
{
    uint32 shift = (uint32)timChannel * IFX_EGTM_TIMINSEL_CH0SEL_LEN;
    uint32 value = (uint32)IFX_EGTM_TIMINSEL_CHSEL_ADC_BOUNDARY_FLAG << shift;
...
IFX_INLINE boolean IfxEgtm_Tim_isFilterOutSignalHigh(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel)
{
	uint32 foutBitPos = (uint32)(1u << (timChannel + IFX_EGTM_CLS_TIM_INP_VAL_F_OUT0_OFF));

	return ((MODULE_EGTM.CLS[cluster].TIM.INP_VAL.U & foutBitPos) != 0u) ? TRUE : FALSE;
}


IFX_INLINE boolean IfxEgtm_Tim_isFilterInputSourceSignalHigh(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel)
{
	uint32 finBitPos = (uint32)(1u << (timChannel + IFX_EGTM_CLS_TIM_INP_VAL_F_IN0_OFF));

	return ((MODULE_EGTM.CLS[cluster].TIM.INP_VAL.U & finBitPos) != 0u) ? TRUE : FALSE;
}


IFX_INLINE boolean IfxEgtm_Tim_isInputSignalHigh(IfxEgtm_Cluster cluster, IfxEgtm_Tim_Ch timChannel)
{
	uint32 inBitPos = (uint32)(1u << (timChannel + IFX_EGTM_CLS_TIM_INP_VAL_TIM_IN0_OFF));

	return ((MODULE_EGTM.CLS[cluster].TIM.INP_VAL.U & inBitPos) != 0u) ? TRUE : FALSE;
}
