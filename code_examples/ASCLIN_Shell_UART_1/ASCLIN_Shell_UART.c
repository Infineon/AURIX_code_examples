...
#define COMMAND_TOGGLE_HELP_TEXT  " : Command to toggle LEDs" ENDL \
                            "         The correct syntax for this command is" ENDL \
...
void printInfo(IfxStdIf_DPipe *io)
{
    ...
    IfxStdIf_DPipe_print(io, "******************************************************************************"ENDL);
    IfxStdIf_DPipe_print(io, "This is an example that shows how to use the Infineon Shell from iLLDs.       "ENDL);
    IfxStdIf_DPipe_print(io, "In order to toggle the LEDs enter the command '" COMMAND_TOGGLE "' followed by one of the "ENDL);
    IfxStdIf_DPipe_print(io, "following parameters:                                                         "ENDL);
    IfxStdIf_DPipe_print(io, "  - '0': turns on all the LEDs on the board                                   "ENDL);
    IfxStdIf_DPipe_print(io, "  - '1': toggles the LED D107                                                 "ENDL);
    IfxStdIf_DPipe_print(io, "  - '2': toggles the LED D108                                                 "ENDL);
    IfxStdIf_DPipe_print(io, "  - '3': toggles the LED D109                                                 "ENDL);
    IfxStdIf_DPipe_print(io, "  - '4': toggles the LED D110                                                 "ENDL);
    IfxStdIf_DPipe_print(io, "Any other parameter turns off all the LEDs and reports a Shell command error. "ENDL);
    IfxStdIf_DPipe_print(io, "******************************************************************************"ENDL);
}
...
boolean toggleLEDsShell(pchar args, void *data, IfxStdIf_DPipe *io)
{
    switch(args[0])
    {
        case '0': /* Turn on all LEDs */
            ...
            IfxStdIf_DPipe_print(io, "Turned on all LEDs!" ENDL ENDL);
            break;
        case '1': /* Toggle LED1 */
            IfxPort_setPinState(LED1, IfxPort_State_toggled);
            IfxStdIf_DPipe_print(io, "Toggled LED1!" ENDL ENDL);
            break;
        case '2': /* Toggle LED2 */
            IfxPort_setPinState(LED2, IfxPort_State_toggled);
            IfxStdIf_DPipe_print(io, "Toggled LED2!" ENDL ENDL);
            break;
        case '3': /* Toggle LED3 */
            IfxPort_setPinState(LED3, IfxPort_State_toggled);
            IfxStdIf_DPipe_print(io, "Toggled LED3!" ENDL ENDL);
            break;
        case '4': /* Toggle LED4 */
            IfxPort_setPinState(LED4, IfxPort_State_toggled);
            IfxStdIf_DPipe_print(io, "Toggled LED4!" ENDL ENDL);
            break;
        default: /* Turn off all LEDs */
            ...
            IfxStdIf_DPipe_print(io, "Command syntax not correct." ENDL \
                    "The correct syntax for this command is" ENDL "    '" COMMAND_TOGGLE " [0/1/2/3/4]'" ENDL \
                    "Turned off all LEDs!" ENDL);
...
void initSerialInterface(void)
{
    ...
    const IfxAsclin_Asc_Pins pins = {
            ...
            .rx         = &IfxAsclin0_RXA_P14_1_IN,         /* Select the pin for RX connected to the micro USB port */
            .rxMode     = IfxPort_InputMode_pullUp,         /* RX pin                                                */
            .rts        = NULL_PTR,                         /* RTS pin not used                                      */
            .rtsMode    = IfxPort_OutputMode_pushPull,
            .tx         = &IfxAsclin0_TX_P14_0_OUT,         /* Select the pin for TX connected to the micro USB port */
...
void initShellInterface(void)
{
    ...
    Ifx_Console_print(ENDL "Enter '" COMMAND_HELP "' to see the available commands" ENDL);
