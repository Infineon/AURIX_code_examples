...
typedef struct
{
...
#elif defined(DEVICE_TC49XN) || defined(DEVICE_TC46X) || defined(DEVICE_TC4EX) 
    ...
    uint8                        descriptorPointer;                   /**< \brief Current descriptor pointer. By default, keep it to 0, valid values 0-3 */
    boolean                      descriptorPointerConfigEnable;       /**< \brief Bypass configuring PTR (current descriptor pointer) using descriptorPointer. FALSE: PTR is not configured, TRUE: PTR is configured */
#if defined(DEVICE_TC4EX) || defined(DEVICE_TC4ZX)
    boolean                      singleDescriptorModeEnable;          /**< \brief Enable Single descriptor mode. False: Disabled. True: DRE disables the preemptive acknowledge of Receive Descriptor for Ethernet to ethernet unicast forward frames based on read by destination Ethernet port from EIBUF and also the number of active descriptors in the Tx descriptor list is limited to 1. */
