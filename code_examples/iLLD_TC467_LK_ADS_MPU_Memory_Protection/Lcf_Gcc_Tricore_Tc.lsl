...
__TRICORE_DERIVATE_MEMORY_MAP__ = 0x460;
...
MEMORY
{
    ...
    prram0 (rx!p): org = 0x80000000, len = 4M
    prram0_nc (rx!p): org = 0xA0000000, len = 4M

    prram1 (rx!p): org = 0x80400000, len = 4M
    prram1_nc (rx!p): org = 0xA0400000, len = 4M

    prram2 (rx!p): org = 0x80800000, len = 2M
    prram2_nc (rx!p): org = 0xA0800000, len = 2M

    prram3 (rx!p): org = 0x80A00000, len = 2M
    prram3_nc (rx!p): org = 0xA0A00000, len = 2M

	
	
    ucb (rx!p): org = 0xae400000, len = 80K

    cpu0_dlmu (w!xp): org = 0x90000000, len = 256K
    cpu0_dlmu_nc (w!xp): org = 0xb0000000, len = 256K

    cpu1_dlmu (w!xp): org = 0x90040000, len = 256K
    cpu1_dlmu_nc (w!xp): org = 0xb0040000, len = 256K

    cpu2_dlmu (w!xp): org = 0x90080000, len = 256K
    cpu2_dlmu_nc (w!xp): org = 0xb0080000, len = 256K

    cpu3_dlmu (w!xp): org = 0x900c0000, len = 128K
    cpu3_dlmu_nc (w!xp): org = 0xb00c0000, len = 128K



		lmuram (w!xp): org = 0x90400000, len = 512K
		lmuram_nc (w!xp): org = 0xb0400000, len = 512K
...
REGION_MIRROR("prram0", "prram0_nc")
REGION_MIRROR("prram1", "prram1_nc")
REGION_MIRROR("prram2", "prram2_nc")
REGION_MIRROR("prram3", "prram3_nc")
...
REGION_ALIAS( default_rom , prram0)

/*
REGION_ALIAS( default_ram , dsram1)
REGION_ALIAS( default_rom , prram1)
*/
/*
REGION_ALIAS( default_ram , dsram2)
REGION_ALIAS( default_rom , prram2)
*/
/*
REGION_ALIAS( default_ram , dsram3)
REGION_ALIAS( default_rom , prram3)
*/
...
/*Sections located at absolute fixed address*/
    ...
    SECTIONS
    {
		.start_tc0 (LCF_STARTPTR_NC_CPU0) : FLAGS(rxl) { KEEP (*(.start)); } > prram0_nc
        .interface_const (0x80000020) : { __IF_CONST = .; KEEP (*(.interface_const)); } > prram0
    ...
    SECTIONS
    {
        .traptab_tc0 (LCF_TRAPVEC0_START) : { PROVIDE(__TRAPTAB_CPU0 = .); KEEP (*(.traptab_cpu0)); } > prram0
        .traptab_tc1 (LCF_TRAPVEC1_START) : { PROVIDE(__TRAPTAB_CPU1 = .); KEEP (*(.traptab_cpu1)); } > prram1
        .traptab_tc2 (LCF_TRAPVEC2_START) : { PROVIDE(__TRAPTAB_CPU2 = .); KEEP (*(.traptab_cpu2)); } > prram2
        .traptab_tc3 (LCF_TRAPVEC3_START) : { PROVIDE(__TRAPTAB_CPU3 = .); KEEP (*(.traptab_cpu3)); } > prram3
    }
    ...
    SECTIONS
    {	
        .start_tc1 (LCF_STARTPTR_NC_CPU1) : FLAGS(rxl)  { KEEP (*(.start_cpu1)); } > prram1_nc
        .start_tc2 (LCF_STARTPTR_NC_CPU2) : FLAGS(rxl)  { KEEP (*(.start_cpu2)); } > prram2_nc
        .start_tc3 (LCF_STARTPTR_NC_CPU3) : FLAGS(rxl)  { KEEP (*(.start_cpu3)); } > prram3_nc
    ...
    SECTIONS
    {
        .usercfg_0_org (0xae400200) : FLAGS(arl) { KEEP (*(.usercfg_0_orig)); } > ucb
        .usercfg_0_cpy (0xae400300) : FLAGS(arl) { KEEP (*(.usercfg_0_copy)); } > ucb
		.bmhd_0_org (0xae400900) : FLAGS(arl) { KEEP (*(.bmhd_0_orig)); } > ucb
        .bmhd_1_org (0xae400a00) : FLAGS(arl) { KEEP (*(.bmhd_1_orig)); } > ucb
        .bmhd_2_org (0xae400b00) : FLAGS(arl) { KEEP (*(.bmhd_2_orig)); } > ucb
        .bmhd_3_org (0xae400c00) : FLAGS(arl) { KEEP (*(.bmhd_3_orig)); } > ucb
...
SECTIONS
{
    CORE_SEC(.text) : FLAGS(axl)
    {
        *(.text.fast.prram.cpu0)
        *(.text.slow.prram.cpu0)
        *(.text.5ms.prram.cpu0)
        *(.text.10ms.prram.cpu0)
        *(.text.callout.prram.cpu0)
        *(.text)
        *(.text.*)
        *(.gnu.linkonce.t.*)
