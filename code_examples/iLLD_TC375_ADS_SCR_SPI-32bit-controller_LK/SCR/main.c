...
__data volatile uint32 g_bitMask;
    ...
    while(1)
    {
        if(g_rtcTrigger)
        {
            ...
            if(g_runSpi)
            {
                ...
                if(0 == g_bitMask)
                {
                    g_bitMask = 0x00000001;
                }

                /* Set next TX Byte */
                g_txBuffer[0] = (uint8)((~g_bitMask) >> 24u);
                g_txBuffer[1] = (uint8)((~g_bitMask) >> 16u);
                g_txBuffer[2] = (uint8)((~g_bitMask) >> 8u);
                g_txBuffer[3] = (uint8)(~g_bitMask);
                g_bitMask <<= 1;
