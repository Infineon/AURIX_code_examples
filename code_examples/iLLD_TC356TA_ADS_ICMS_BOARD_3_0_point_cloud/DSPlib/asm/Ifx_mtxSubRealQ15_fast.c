...
#define     MSubCnt     _A2       /* Loop-Cnt-Reg						*/
...
void Ifx_mtxSubRealQ15_fast(struct Ifx_mtxRealQ15State * state){
	/* Fetch Inputs
	 *	sint16 * sr1;  	  --> offset 0
	 *	sint16 * sr2; 	  --> offset 4
    ...
    MOV_A(MSubCnt, dTmp);        	/* Transfer count to Loop-Counter			*/

IFXASM_LABEL(MatSub_SubL);
    SUBS_H(sseR, sseX, ssoY);      	/* sseR (lo) =  X[j] - Y[j]
                                   	   sseR (hi) = X[j+1] - Y[j+1]					*/
    LD_D(ssssY,[aY+]8);		    	/* ssssY <- next four values of matrix Y		*/
    SUBS_H(ssoR, ssoX, sseY);       /* ssoR (lo) =  X[j+2] - Y[j+2]
                                       ssoR (hi) = X[j+3] - Y[j+3]					*/
    LD_D(ssssX,[aX+]8);    			/* ssssX <- next four values of matrix X		*/
    ST_D([aR+]8,ssssR);    			/* Results of diff of four corresponding
                            		   entries of X and Y stored to R				*/
    LOOP(MSubCnt, MatSub_SubL);

    RET();
}

/* Faster version for cases where nCols*nRows is divisible by 4	*/
void Ifx_mtxSubRealQ15_fast4N(struct Ifx_mtxRealQ15State * state){
...
IFXASM_LABEL(MatSub_SubL4N);
    SUBS_H(_D8, _D2, _D6);      	/* D8 (lo) =  X[j] - Y[j]
                                   	   D8 (hi) = X[j+1] - Y[j+1]				*/
    LD_D(_E10,[aX+]8);		    	/* E10 <- next four values of matrix Y		*/
    SUBS_H(_D9, _D3, _D7);       	/* D9 (lo) =  X[j+2] - Y[j+2]
                                       D9 (hi) = X[j+3] - Y[j+3] 				*/
    LD_D(_E12,[aY+]8);    			/* E12 <- next four values of matrix Y		*/
    ST_D([aR+]8,_E8);    			/* Results of diff of four corresponding
                            		   entries of X and Y stored to R			*/
    SUBS_H(_D8, _D10, _D12);      	/* D8 (lo) =  X[j+4] - Y[j+4]
                                   	   D8 (hi) = X[j+5] - Y[j+5]				*/
    LD_D(_E2,[aX+]8);		    	/* E2 <- next four values of matrix Y		*/
    SUBS_H(_D9, _D11, _D13);        /* D9 (lo) =  X[j+6] - Y[j+6]
                                       D9 (hi) = X[j+7] - Y[j+7] 				*/
    LD_D(_E6,[aY+]8);    			/* E6 <- next four values of matrix Y		*/
    ST_D([aR+]8,_E8);    			/* Results of diff of four corresponding
                                		entries of X and Y stored to R			*/
    LOOP(MSubCnt, MatSub_SubL4N);
