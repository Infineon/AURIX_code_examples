...
#define     lAbs        _D3        /* Stores |X| */
#define     lXbyPi      _D4        /* Stores |X|/pi or (pi - |X|)/pi */
...
#define     dTmp1       _D15       /* Generic temporary Data-Reg */
#define     dTmp2       _D8        /* Generic temporary Data-Reg */
...
__asm
(
	"ConstTab_3:   .word   0x3243F6A9\n"	/* pi/2 in 3Q29 format */
	"			.word   0x6487ED51\n"		/* Constant pi in 3Q29 format */
	"			.word   0x28BE60DB\n"		/* 1/pi in 1Q31 format */
	"			.word   0x1CCE0008\n"		/* 1.800293   - H(5) */
	"			.word   0x08B70011\n"		/* 0.5446778  - H(4) */
	"			.word   0xAACBFF48\n"		/* -5.325196  - H(3) */
...
sint16 Ifx_sinQ15(sint32 x){
	...
	ABS(lAbs,lX);
	MOVH_A(aH,_HI(ConstTab_3));
	LEA(aH,[aH]_LO(ConstTab_3));
	NE(lSign,lX,lAbs);
	LD_W(lPi_2,[aH+]2*W16);
	LT(dTmp1,lPi_2,lAbs);
	LD_W(lPi,[aH+]2*W16);
	CSUB(dTmp2,dTmp1,lPi,lAbs);
	LD_W(l1_Pi,[aH+]2*W16);
	CMOV(lAbs,dTmp1,dTmp2);
