...
sint16 Ifx_maxposRealQ15 (sint16 *Xa, uint16 nXa, uint16 *posa){
...
IFXASM_LABEL(VecloopMxv_1);
	IXMAX(ssResReg, ssCmpReg, ssCmpVal);
...
IFXASM_LABEL(Vecmx_odd);
	LD_H(ssCmpVal,[aX]);
	SH(ssCmpVal,ssCmpVal,_N(16));
	SH(ssCmpVal,ssCmpVal,_N(-16));
	ADD_2ARGS(nX,_N(-0x1));
	LT_H(dTmp,ssResReg1,ssCmpVal);
