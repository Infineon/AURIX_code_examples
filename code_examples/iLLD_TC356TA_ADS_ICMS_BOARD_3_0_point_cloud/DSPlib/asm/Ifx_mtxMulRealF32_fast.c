...
void Ifx_mtxMulRealF32_fast(struct Ifx_mtxMulRealF32State * state)
{
	/**
	  	enum Ifx_mode mode;   --> offset 0
		float32 * dst;  	  --> offset 4
		float32 * src1; 	  --> offset 8
		float32 * src2; 	  --> offset 12
		uint32 m;       	  --> offset 16
		uint32 n;			  --> offset 20
		uint32 l;			  --> offset 24
	 */
...
IFXASM_LABEL(L);
	...
	* Mid Loop */
	...
	ST_D([aR+]W32, llRri);
	ADDSC_A(aY0, aY0, dC4, _N(0));
	MOV_AA(aY, aY0);				 /* Transfer aY0 to aY */
	LOOP(anColY, N);

	/* ------------------- Outer loop ends ------------------------------------------
	* Outer Loop M */
	ADDSC_A(aX0, aX0, nColX, _N(2));
	JNED(nRowX, _N(0), M);
