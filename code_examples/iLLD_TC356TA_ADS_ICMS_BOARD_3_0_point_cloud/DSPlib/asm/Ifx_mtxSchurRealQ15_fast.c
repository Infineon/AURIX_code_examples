...
#define     sseYul      _D6UL     /*   variant for MUL.R instr.			*/
#define     ssoY        _D7       /* Odd-Reg							*/
#define     ssoYul      _D7UL     /*   variant for MUL.R instr.			*/
...
void Ifx_mtxSchurRealQ15_fast(struct Ifx_mtxRealQ15State * state){
	/* Fetch Inputs
	 *	sint16 * sr1;  	  --> offset 0
	 *	sint16 * sr2; 	  --> offset 4
...
IFXASM_LABEL(MatAdd_AddL);
    MULR_H(sseR, sseX, ssoYul, _N(1));	/* sseR (lo) =  X[j] * Y[j]
                                   	       sseR (hi) = X[j+1] * Y[j+1]						*/
    LD_D(ssssY,[aY+]8);		    		/* ssssY <- next four values of matrix Y		*/
    MULR_H(ssoR, ssoX, sseYul, _N(1)); 	/* ssoR (lo) =  X[j+2] * Y[j+2]
                                           ssoR (hi) = X[j+3] * Y[j+3]						*/
    LD_D(ssssX,[aX+]8);    				/* ssssX <- next four values of matrix X		*/
    ST_D([aR+]8,ssssR);    				/* Results of product of four corresponding
...
void Ifx_mtxSchurRealQ15_fast4N(struct Ifx_mtxRealQ15State * state){
	/* Fetch Inputs
	 *	sint16 * sr1;  	  --> offset 0
	 *	sint16 * sr2; 	  --> offset 4
	 *	sint16 * dest;	  --> offset 8
...
IFXASM_LABEL(MatAdd_AddL4N);
	MULR_H(_D8, _D2, _D6UL, _N(1));     /* D8 (lo) =  X[j] * Y[j]
                                   	       D8 (hi) = X[j+1] * Y[j+1]					*/
    LD_D(_E10,[aX+]8);		    		/* E10 <- next four values of matrix Y		*/
    MULR_H(_D9, _D3, _D7UL, _N(1));     /* D9 (lo) =  X[j+2] * Y[j+2]
                                           D9 (hi) = X[j+3] * Y[j+3] 					*/
    LD_D(_E12,[aY+]8);    				/* E12 <- next four values of matrix Y		*/
    ST_D([aR+]8,_E8);    				/* Results of product of four corresponding
                            		       entries of X and Y stored to R				*/
    MULR_H(_D8, _D10, _D12UL, _N(1));   /* D8 (lo) =  X[j+4] * Y[j+4]
                                   	       D8 (hi) = X[j+5] * Y[j+5]					*/
    LD_D(_E2,[aX+]8);		    		/* E2 <- next four values of matrix Y		*/
    MULR_H(_D9, _D11, _D13UL, _N(1));  	/* D9 (lo) =  X[j+6] * Y[j+6]
                                           D9 (hi) = X[j+7] * Y[j+7] 					*/
