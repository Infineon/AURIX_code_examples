...
/* This SPU configuration applies a range FFT along samples, windowing, calibration and rejects the symmetric half of range spectrum*/

IfxSpu_ConfigSlot *IfxSpu_cfg2_rFFT(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    ...
    cfg->input.rm.baseAddress         = IFX_OFFSETOF(SPU_Rmem_Map_t, adc_sort[0]);
    cfg->input.rm.dataFormat         = IfxSpu_RM_InputDataFormat_real16Bit;

    cfg->input.rm.antennaMapping     = IfxSpu_RM_AntennaMapping_indexMode; /*antenna idx updated after each full inner loop*/
    ...
    cfg->input.rm.innerLoopRepeat     = RADAR_NUM_RX*2;
    cfg->input.rm.outerLoopRepeat     = RADAR_NUM_RAMPS/2;

    cfg->input.rm.binLoopOffset       = bin_size_bytes;
    cfg->input.rm.innerLoopOffset     = bin_size_bytes * RADAR_NUM_SAMPLES;
    cfg->input.rm.outerLoopOffset     = bin_size_bytes * RADAR_NUM_SAMPLES * RADAR_NUM_RX*2;
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        /* MATH1 */
       pcfg0->math1.loaderExponent         = 32-RADAR_NUM_ADC_BITS-1; /*alignment when reformatting input data to 32bit*/
       ...
       pcfg0->math1.window.baseAddress      = IFX_OFFSETOF(SPU_Cmem0_Map_t, calibWndw[0][0]);
       pcfg0->math1.window.dataFormat         = IfxSpu_WindowDataFormat_complex32;
       pcfg0->math1.phaseShift             = IfxSpu_PhaseShift_0;
       for(uint32 antNr=0; antNr<RADAR_NUM_VRX; antNr++)
       {
           volatile uint32 offsetused = IFX_OFFSETOF(SPU_Cmem0_Map_t, calibWndw[antNr][0]) - pcfg0->math1.window.baseAddress;
             pcfg0->math1.window.antennaOffsets[antNr] =IFX_OFFSETOF(SPU_Cmem0_Map_t, calibWndw[antNr][0]) - pcfg0->math1.window.baseAddress;
       }
       IfxSpu_setupMath1(slot, passId, &pcfg0->math1);
        ...
        pcfg0->fft.size         = IfxSpu_FftSize_256;
        ...
        pcfg0->fftOut.baseAddress     = IFX_OFFSETOF(SPU_Rmem_Map_t, rMapcalibrated[0]);
        pcfg0->fftOut.format         = IfxSpu_ODP_Format_complex16Bit;
        pcfg0->fftOut.exponent         = 13;
