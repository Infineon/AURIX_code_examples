...
/*This SPU config calculates the sum across ramps of a RADAR frame and writes the result to the micro doppler frame*/

IfxSpu_ConfigSlot *IfxSpu_cfg4_sum_ramps(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    ...
    cfg->input.rm.baseAddress         = IFX_OFFSETOF(SPU_Rmem_Map_t, rMapcalibrated[0]);
    cfg->input.rm.dataFormat         = IfxSpu_RM_InputDataFormat_complex16Bit;

    cfg->input.rm.antennaMapping     = IfxSpu_RM_AntennaMapping_innerLoopRepeat; /*antenna idx updated after each full inner loop*/
    ...
    cfg->input.rm.binLoopRepeat       = RADAR_NUM_SAMPLES/2;
    cfg->input.rm.innerLoopRepeat     = RADAR_NUM_RAMPS/2;
    cfg->input.rm.outerLoopRepeat     = RADAR_NUM_VRX;

    cfg->input.rm.binLoopOffset       = bin_size_bytes;
    cfg->input.rm.innerLoopOffset     = bin_size_bytes * RADAR_NUM_SAMPLES/2*RADAR_NUM_VRX;
    cfg->input.rm.outerLoopOffset     = bin_size_bytes * RADAR_NUM_SAMPLES/2;
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        /* MATH1 */
        pcfg0->math1.loaderExponent         = 16; /*alignment when reformatting input data to 32bit*/
        ...
        pcfg0->fft.size         = IfxSpu_FftSize_128;
        ...
        /*MATH2*/
        /*summation for complex average across ramps (8 ramps set on inner loop and declared to be antenna)*/
        pcfg0->csum.baseAddress       = IFX_OFFSETOF(SPU_Rmem_Map_t, microDoppler[RADAR_NUM_MICRO_DOPPLER-1]);
        pcfg0->csum.mode              = IfxSpu_Summation_Mode_sumAnt;
        pcfg0->csum.dataFormat        = IfxSpu_Summation_DataFormat_complex;
        pcfg0->csum.antennaeToUse     = 0xFF;
