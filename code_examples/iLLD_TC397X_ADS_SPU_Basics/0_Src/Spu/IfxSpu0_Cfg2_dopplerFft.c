/**********************************************************************************************************************
 * \file IfxSpu0_Cfg2_dopplerFFT.c
...
IfxSpu_ConfigSlot *IfxSpu0_Cfg2_dopplerFFT(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    uint32             antNr;
    ...
    /* GLOBAL */
    {
        ...
        cfg->input.rm.baseAddress  = IFX_OFFSETOF(SPU_Emem_Map_t, rangeMap.rMap[0]);
        ...
        cfg->input.rm.binLoopRepeat   = RADAR_NUM_RAMPS;
        cfg->input.rm.innerLoopRepeat = RADAR_NUM_RX;
        cfg->input.rm.outerLoopRepeat = RADAR_NUM_RANGE_GATES;
        cfg->input.rm.binLoopOffset   = inputBinSize * RADAR_NUM_RANGE_GATES * RADAR_NUM_RX;
        cfg->input.rm.innerLoopOffset = inputBinSize * RADAR_NUM_RANGE_GATES;
        ...
        cfg->localMax.localMaxMode   = IfxSpu_LocalMax_Mode_under; /* ignore bins below threshold */
        cfg->localMax.thresholdMode  = IfxSpu_LocalMax_ThresholdMode_under; /* ignore bins below threshold */
        cfg->localMax.thresholdValue = 0xFFFF;
    ...
    /* FIRST PASS */
    {
        ...
        /* MATH 1 */
        pcfg->math1.loaderExponent      = 16; /*alignment when reformatting FFT1 data to 32bit*/
        pcfg->math1.numDropFirstSamples = 0;
        pcfg->math1.numDropLastSamples  = 0;
        pcfg->math1.numPadFrontSamples  = 0;
        pcfg->math1.window.enabled      = TRUE;
        pcfg->math1.window.dataFormat   = IfxSpu_WindowDataFormat_real16;
        pcfg->math1.window.baseAddress  = IFX_OFFSETOF(SPU_Cmem0_Map_t, dWndw);
        ...
        pcfg->fft.size        = IfxSpu_getSizeCode(RADAR_FFT2_LEN);
        ...
        pcfg->fftOut.baseAddress   = IFX_OFFSETOF(SPU_Emem_Map_t, rdMap[0]);
        ...
        pcfg->nci.baseAddress      = IFX_OFFSETOF(SPU_Emem_Map_t, nci[0]);
        pcfg->nci.enabled          = TRUE;
        pcfg->nci.outputFormat     = IfxSpu_NCI_OutputFormat_real32Bit;
        ...
        /* COMMON CFAR */
        pcfg->cfar.baseAddress = IFX_OFFSETOF(SPU_Emem_Map_t, lclMaxVMap[0]);
