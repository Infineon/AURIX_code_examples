...
IfxSpu_ConfigSlot *IfxSpu_cfg5_sub(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    ...
    cfg->input.rm.innerLoopOffset     = bin_size_bytes * RADAR_NUM_RX * RADAR_NUM_RANGE_GATES * RADAR_NUM_RAMPS; /*offset corresponding to rmap size*/
    cfg->input.rm.outerLoopOffset     = bin_size_bytes * RADAR_NUM_RX * RADAR_NUM_RANGE_GATES;
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        pcfg0->csum.baseAddress   = IFX_OFFSETOF(SPU_Emem_Map_t, rMapCR[0]); /*can alternatively be set to overwrite original Range map*/
