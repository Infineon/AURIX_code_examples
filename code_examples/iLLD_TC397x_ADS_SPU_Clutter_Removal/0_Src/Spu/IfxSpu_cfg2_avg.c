...
IfxSpu_ConfigSlot *IfxSpu_cfg2_avg(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    ...
    cfg->input.rm.innerLoopRepeat   = RADAR_NUM_RANGE_GATES;
    cfg->input.rm.outerLoopRepeat   = RADAR_NUM_RX;

    cfg->input.rm.binLoopOffset     = bin_size_bytes * RADAR_NUM_RANGE_GATES * RADAR_NUM_RX;
    cfg->input.rm.innerLoopOffset   = bin_size_bytes;
    cfg->input.rm.outerLoopOffset   = bin_size_bytes * RADAR_NUM_RANGE_GATES;
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        pcfg0->math1.phaseShift             = IfxSpu_PhaseShift_180; /* Phase 180° = multiplies factor of -1 for subtraction */
        ...
        /*MATH2*/
        /*summation for average calculation (clutter removal)*/
        pcfg0->csum.baseAddress       = IFX_OFFSETOF(SPU_Emem_Map_t, average32[0]);
        pcfg0->csum.mode              = IfxSpu_Summation_Mode_sum;
        pcfg0->csum.dataFormat        = IfxSpu_Summation_DataFormat_complex;
        pcfg0->csum.shift             = IfxSpu_Summation_Scale_off;
        pcfg0->csum.precision         = IfxSpu_Summation_Precision_32bit;
