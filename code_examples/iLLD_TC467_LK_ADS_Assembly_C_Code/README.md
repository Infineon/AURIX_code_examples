...
# iLLD_TC467_LK_ADS_Assembly_C_Code

**Inline assembler and assembler files are used in combination in a C project.**  

## Device  
The device used in this example is AURIX&trade; TC467QP_A-Step_MS_STD  

## Board  
The board used for testing is the AURIX&trade; TC467 lite Kit (KIT_A3G_TC467_LITE)  
...
## Hardware setup  
This code example has been developed for the board AURIX&trade; TC467 lite Kit (KIT_A3G_TC467_LITE):  
 
<img src="./Images/KIT_TC467_LK_Front.png" width="800" />  

## Implementation    
**LED1 state control**  

LED1 driven by port 22 pin 7 can be switched ON/OFF by calling the assembly function *set_LED1_State_Assembly()*, implemented in the assembly source file *Assembly_Code.s*.
...
- Load the Port 22 Output Modification Register (OMR) into A0 address register
- Store D0 value into A0 address register (Port 22 OMR register)
- Return from function, needed to restore the context of the caller function

**LED2 state control**  

LED2 driven by port 22 pin 6 can be switched ON/OFF by calling the assembly function *set_LED2_State_Assembly()*, implemented in the C source *Assembly_C_Code.c* file using the *__asm()__* *volatile* keyword.

This functionality is ensured by the following steps:
- Check the value passed to the function:
    - If 0 (LED_OFF): write 0x20 to D0 data register
    - Else 1 (LED_ON): write 0x20 to D0 data register
- Load the Port 22 OMR register into A0 address register
- Store D0 value into A0 address register (Port 22 OMR register)
...
**Scenario:**
- P22.7 and P22.6 are used to drive the LED1 and LED2 
...
- Firstly, LED1 (P22.7) is switched on
- One second after, LED2 (P22.6) is switched on
- One second after, LED1 (P22.7) is switched off
- One second after, LED2 (P22.6) is switched off
  
<img src="./Images/LED.gif" width="800" />  
