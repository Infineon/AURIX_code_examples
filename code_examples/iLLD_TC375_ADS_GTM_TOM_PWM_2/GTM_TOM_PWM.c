...
#define PWM_OUT                  &IfxGtm_TOM1_4_TOUT14_P00_5_OUT       /* LED which is driven by the PWM, P00.5      */
#define PWM_MAX_DUTY            (100.0f)                               /* Maximum PWM duty cycle for the TOM         */
#define FADE_STEP               (PWM_MAX_DUTY / 100.0f)                /* PWM duty cycle step for the TOM            */
...
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
IFX_STATIC GtmTomPwm g_gtmTomPwm;                               /* Structure for PWM generation                      */
...
/* This function initializes the TOM PWM */
void initGtmTomPwm(void)
{
    ...
    channelConfig[0].timerCh      = IfxGtm_Pwm_SubModule_Ch_4;          /* Tom channel index                         */
    ...
    pwmConfig.cluster                = IfxGtm_Cluster_1;                /* Cluster                                   */
    pwmConfig.subModule              = IfxGtm_Pwm_SubModule_tom;        /* Sub module                                */
    ...
    pwmConfig.clockSource.tom        = IfxGtm_Cmu_Fxclk_0;              /* Clock source for Tom                      */
    ...
    IfxGtm_Pwm_init(&g_gtmTomPwm.pwm, &g_gtmTomPwm.channels[0], &pwmConfig);

    /* Store the current duty value for runtime calls */
    g_gtmTomPwm.dutyCycles[0] = channelConfig[0].duty;
}
...
{
    g_gtmTomPwm.dutyCycles[0] = dutyCycle;                          /* Set PWM duty cycle                            */

    /* Update duty of all configured channels to requested values immediately */
    IfxGtm_Pwm_updateChannelsDutyImmediate(&g_gtmTomPwm.pwm, (float32*)g_gtmTomPwm.dutyCycles);
}
