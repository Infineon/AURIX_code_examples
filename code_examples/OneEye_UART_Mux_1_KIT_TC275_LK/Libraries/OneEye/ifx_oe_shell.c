...
#define IFX_OE_SHELL_LLD "%lld "
#define IFX_OE_SHELL_LLX "%llx "
#define IFX_OE_SHELL_LLU "%llu "
 ...
 */
void                        Ifx_Oe_Shell_cmdEscapeProcess(Ifx_Oe_Shell* shell, char EscapeChar1, char EscapeChar2);
const Ifx_Oe_Shell_Command* Ifx_Oe_Shell_commandListFind(Ifx_Oe_Shell* shell, pchar commandLine, pchar* args, Ifx_Oe_Shell_CommandListConst* commandList);
 ...
 */
IFX_OE_INLINE boolean Ifx_Oe_Shell_isEndOfLine(pchar args)
{
    return ((args == NULL_PTR) || (*args == IFX_OE_SHELL_NULL_CHAR)) ? TRUE : FALSE;
}
...
boolean Ifx_Oe_Shell_showHelpSingle(Ifx_Oe_Shell_CommandListConst commandList, IfxStdIf_DPipe* io, boolean briefOnly, boolean singleCommand)
{
    ...
    while (command->commandLine != NULL_PTR)
    {
        ...
        while (*help != IFX_OE_SHELL_NULL_CHAR)
        {
            ...
            if (Ifx_Oe_Shell_matchCommand(&help, &matchp))
            ...
            else if (Ifx_Oe_Shell_matchCommand(&help, &matchs))
            {   /* Parameter description found */
                isParam    = FALSE;
                isSyntax   = TRUE,
                spaceParam = "    SYNTAX: ";
            ...
            else if (Ifx_Oe_Shell_matchCommand(&help, &matchSection))
            {   /* Parameter description found */
                isParam      = FALSE;
                isSyntax     = FALSE;
                sectionLevel = 1;
            ...
            else if (Ifx_Oe_Shell_matchCommand(&help, &matchSubSection))
            {   /* Parameter description found */
                isParam      = FALSE;
                isSyntax     = FALSE;
                sectionLevel = 2;
            ...
            /* Split print by IFX_OE_ENDL */
            chunk = strstr(help, IFX_OE_ENDL);
...
boolean Ifx_Oe_Shell_showHelpSingleCommand(pchar args, void* shellPtr, IfxStdIf_DPipe* io)
{
    ...
    if (shellCommand != NULL_PTR)
    {
        ...
        {
            Ifx_Oe_Shell_showHelpSingle(commandList, io, FALSE, FALSE);
        }
        else
        {
...
boolean Ifx_Oe_Shell_showHelp(pchar args, void* shellPtr, IfxStdIf_DPipe* io)
{
    ...
    {
        for (i = 0; i < IFX_CFG_OE_SHELL_COMMAND_LISTS; i++)
        {
            if (shell->commandList[i] != NULL_PTR)
            {
                Ifx_Oe_Shell_showHelpSingle(shell->commandList[i], io, TRUE, FALSE);
            }
        }
    }
    else
    {
        result = Ifx_Oe_Shell_showHelpSingleCommand(args, shellPtr, io);
    }
...
boolean Ifx_Oe_Shell_bbProtocolStart(pchar args, void* data, IfxStdIf_DPipe* io)
{
    ...
    {
        result = Ifx_Oe_Shell_protocolStart(args, data, io);
    }
    else
    {
    }
...
//---------------------------------------------------------------------------
void Ifx_Oe_Shell_initConfig(Ifx_Oe_Shell_Config* config)
{
    uint32 i;
...
boolean Ifx_Oe_Shell_init(Ifx_Oe_Shell* shell, const Ifx_Oe_Shell_Config* config)
{
    sint32 i;
    ...
    shell->locals.escBracketNum   = IFX_OE_SHELL_NULL_CHAR; /* Used to cache number in sequence "ESC [ 1/2/3/4 ~" */
    shell->locals.cmdState        = Ifx_Oe_Shell_CmdState_normal;
    ...
    memset(Ifx_Oe_Shell_cmdBuffer, 0, sizeof(Ifx_Oe_Shell_cmdBuffer));
    shell->cmdHistory[0] = &Ifx_Oe_Shell_cmdBuffer[0];
    ...
    for (i = 1; i < IFX_CFG_OE_SHELL_CMD_HISTORY_SIZE; i++)
    {
        CmdHistory[i] = &CmdHistory[i - 1][IFX_CFG_OE_SHELL_CMD_LINE_SIZE];    /* Items are just offsets into a large allocated area */
    }

    shell->cmd.historyItem = IFX_OE_SHELL_CMD_HISTORY_NO_ITEM;
...
void Ifx_Oe_Shell_prefill(Ifx_Oe_Shell* shell, const char* text)
{
    Ifx_Oe_Shell_CmdLine* Cmd    = &shell->cmd;
    char*                 cmdStr = shell->locals.cmdStr;
    Ifx_Oe_SizeT          i;           /* Loop variables */
    Ifx_Oe_SizeT          count  = (Ifx_Oe_SizeT)strlen(text);
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (Cmd->length < (IFX_CFG_OE_SHELL_CMD_LINE_SIZE - 1))
        {
...
Ifx_Oe_Shell_SpecialKey Ifx_Oe_Shell_process(Ifx_Oe_Shell* shell)
{
    Ifx_Oe_SizeT            i, j;         /* Loop variables */
    Ifx_Oe_SizeT            count;
    Ifx_Oe_SizeT            readCount;
    boolean                 NormalKeyPress; /* Indicates if this is a normal keypress, i.e. not part of an escape code */
    Ifx_Oe_Shell_SpecialKey result    = Ifx_Oe_Shell_SpecialKey_null;
    ...
    else
    {
        ...
        /* If the command is bigger than IFX_OE_SHELL_CMD_SIZE, the command is ignored.    */
        /*                                                                              */
        ...
        for (i = 0; i < count; i++)
        {
            ...
            case '\r':
                ...
                if (Cmd->length < IFX_CFG_OE_SHELL_CMD_LINE_SIZE)
                {
                    cmdStr[Cmd->length] = IFX_OE_SHELL_NULL_CHAR;  /* Terminate cmdStr */

                    if (Cmd->historyAdd != FALSE)
                    {
                        /* Shuffle history up */
                        for (j = IFX_CFG_OE_SHELL_CMD_HISTORY_SIZE - 1; j > 0; j--)
                        {
                            /* Copy text */
                            strncpy(CmdHistory[j], CmdHistory[j - 1], IFX_CFG_OE_SHELL_CMD_LINE_SIZE);
                        }

                        /* Copy in new entry */
                        strncpy(CmdHistory[0], cmdStr, IFX_CFG_OE_SHELL_CMD_LINE_SIZE);
                        result = Ifx_Oe_Shell_SpecialKey_cr;
                    }

                    if (shell->control.execute == 1)
                    {
                        /* Execute command */
                        if (Ifx_Oe_Shell_execute(shell, cmdStr) == 0)
                        {
                            result = Ifx_Oe_Shell_SpecialKey_ok;
                        }
                    }
                }
                ...
                Cmd->historyItem = IFX_OE_SHELL_CMD_HISTORY_NO_ITEM;
                break;
            ...
            case '\b':
                ...
                if (Cmd->cursor > 0)
                {
                    ...
                    cmdStr[Cmd->length - 1] = IFX_OE_SHELL_NULL_CHAR;
            ...
            case '\x03':
                ...
                result = Ifx_Oe_Shell_SpecialKey_ctrlC;

                break;
            /* Escape character */
            case '\x1B':       /*'\x1B': */
                shell->locals.cmdState = Ifx_Oe_Shell_CmdState_escape;
                break;

            /* '[' - check to see if this is second part of an escape sequence */
            case '[':

                if (shell->locals.cmdState == Ifx_Oe_Shell_CmdState_escape)
                {
                    /* ESC [ pressed */
                    shell->locals.cmdState = Ifx_Oe_Shell_CmdState_escapeBracket;
                }
                else
                {
                    NormalKeyPress = TRUE;
            ...
            case 'D':
                ...
                if (shell->locals.cmdState == Ifx_Oe_Shell_CmdState_escapeBracket)
                {
                    /* Process arrow keys */
                    Ifx_Oe_Shell_cmdEscapeProcess(shell, inputbuffer[i], 0);
            ...
            case '4':
                ...
                if (shell->locals.cmdState == Ifx_Oe_Shell_CmdState_escapeBracket)
                {
                    ...
                    shell->locals.cmdState      = Ifx_Oe_Shell_CmdState_escapeBracketNumber;
                }
                else
                {
                    NormalKeyPress = TRUE;
            ...
            case '~':
                ...
                if (shell->locals.cmdState == Ifx_Oe_Shell_CmdState_escapeBracketNumber)
                {
                    /* Process home/delete/end */
                    Ifx_Oe_Shell_cmdEscapeProcess(shell, shell->locals.escBracketNum, '~');
            ...
            IFX_OE_ASSERT(Cmd->length >= Cmd->cursor);     /* Sanity check */
...
void Ifx_Oe_Shell_deinit(Ifx_Oe_Shell* shell)
{
...
pchar Ifx_Oe_Shell_skipWhitespace(pchar args)
{
    if (args != NULL_PTR)
    {
        while ((*args != IFX_OE_SHELL_NULL_CHAR) && (IFX_OE_SHELL_ISSPACE(*args)))
        {
            args = &args[1];
        }
    }
...
boolean Ifx_Oe_Shell_matchToken(pchar* argsPtr, pchar token)
{
    pchar   savedArguments = *argsPtr;
    char    buffer[IFX_CFG_OE_SHELL_CMD_LINE_SIZE];
    boolean result         = FALSE;

    if (Ifx_Oe_Shell_parseToken(argsPtr, buffer, IFX_OE_COUNTOF(buffer)) != FALSE)
    {
...
boolean Ifx_Oe_Shell_matchCommand(pchar* argsPtr, pchar* match)
{
    boolean result         = FALSE;
    pchar   savedArguments = *argsPtr;
    pchar   savedMatch     = *match;
    char    buffer0[IFX_CFG_OE_SHELL_CMD_LINE_SIZE];
    char    buffer1[IFX_CFG_OE_SHELL_CMD_LINE_SIZE];

    if ((Ifx_Oe_Shell_parseToken(argsPtr, buffer0, IFX_OE_COUNTOF(buffer0)) != FALSE)
        && (Ifx_Oe_Shell_parseToken(match, buffer1, IFX_OE_COUNTOF(buffer1)) != FALSE))
    {
...
boolean Ifx_Oe_Shell_parseToken(pchar* argsPtr, char* tokenBuffer, int bufferLength)
{
    int   mindex = 0;
    ...
    if (*args == '\"')
    {
        ...
        while ((*args != IFX_OE_SHELL_NULL_CHAR) && (*args != '\"'))
        {
...
boolean Ifx_Oe_Shell_parseAddress(pchar* argsPtr, void** address)
{
    char    buffer[32];
    boolean result;
    ...
    *address = 0;
    ...
    {
        result = (buffer[0] != IFX_OE_SHELL_NULL_CHAR) && (sscanf(buffer, "%x ", (unsigned int*)address) == 1);
    }
...
boolean Ifx_Oe_Shell_parseSInt32(pchar* argsPtr, sint32* value)
{
    sint64  value64;
    boolean result;
    ...
    *value = 0;
    ...
    if (Ifx_Oe_Shell_parseSInt64(argsPtr, &value64) == FALSE)
    {
        result = FALSE;
    }
    else
...
boolean Ifx_Oe_Shell_parseUInt32(pchar* argsPtr, uint32* value, boolean hex)
{
    uint64  value64;
    boolean result;
    ...
    *value = 0;
    ...
    if (Ifx_Oe_Shell_parseUInt64(argsPtr, &value64, hex) == FALSE)
    {
        result = FALSE;
    }
    else
...
boolean Ifx_Oe_Shell_parseSInt64(pchar* argsPtr, sint64* value)
{
    char    buffer[64];
    boolean result;
    ...
    *value = 0;
    ...
    {
        result = (buffer[0] != IFX_OE_SHELL_NULL_CHAR) && (sscanf(buffer, IFX_OE_SHELL_LLD, value) == 1);
    }
...
boolean Ifx_Oe_Shell_parseUInt64(pchar* argsPtr, uint64* value, boolean hex)
{
    char    buffer[64];
    boolean result;
    ...
    *value = 0;
    ...
    else
    {
        ...
        if (hex != FALSE)
        {
            result = (bufferPointer[0] != IFX_OE_SHELL_NULL_CHAR) && (sscanf(bufferPointer, IFX_OE_SHELL_LLX, value) == 1);
        }
        else
        {
            result = (bufferPointer[0] != IFX_OE_SHELL_NULL_CHAR) && (sscanf(bufferPointer, IFX_OE_SHELL_LLU, value) == 1);
        }
    }
...
boolean Ifx_Oe_Shell_parseFloat64(pchar* argsPtr, float64* value)
{
    char    buffer[64];
    boolean result;
    ...
    *value = 0;
    ...
    {
        result = (buffer[0] != IFX_OE_SHELL_NULL_CHAR) && (sscanf(buffer, "%lf ", value) == 1);
    }
...
boolean Ifx_Oe_Shell_parseFloat32(pchar* argsPtr, float32* value)
{
    char    buffer[64];
    boolean result;
    ...
    *value = 0;
    ...
    {
        result = (buffer[0] != IFX_OE_SHELL_NULL_CHAR) && (sscanf(buffer, "%f ", value) == 1);
    }
...
const Ifx_Oe_Shell_Command* Ifx_Oe_Shell_commandFind(Ifx_Oe_Shell_CommandListConst commandList, pchar commandLine, pchar* args, uint32* match)
{
    const Ifx_Oe_Shell_Command* command     = commandList;
    const Ifx_Oe_Shell_Command* result      = NULL_PTR;
    ...
    *match    = 0;
    ...
    while (command->commandLine != NULL_PTR)
    {
        ...
        while (Ifx_Oe_Shell_matchCommand(&commandLineTemp, &commandTemp) != FALSE)
        {
            matchCount++;
        }

        if ((matchCount > *match) && (Ifx_Oe_Shell_parseToken(&commandTemp, buffer, IFX_OE_COUNTOF(buffer)) == FALSE))
        {
...
{
    int                         i;
    const Ifx_Oe_Shell_Command* shellCommand = NULL_PTR;
    const Ifx_Oe_Shell_Command* Command      = NULL_PTR;
    uint32                      matchMax     = 0;
    uint32                      match;
    ...
    for (i = 0; i < IFX_CFG_OE_SHELL_COMMAND_LISTS; i++)
    {
        if (shell->commandList[i] != NULL_PTR)
        {
            shellCommand = Ifx_Oe_Shell_commandFind(shell->commandList[i], commandLine, args, &match);
...
Ifx_Oe_Shell_ExecuteCode Ifx_Oe_Shell_execute(Ifx_Oe_Shell* shell, pchar commandLine)
{
    Ifx_Oe_Shell_ExecuteCode      result       = Ifx_Oe_Shell_ExecuteCode_notFound;
    pchar                         args         = NULL_PTR;
    Ifx_Oe_Shell_CommandListConst commandList;
    const Ifx_Oe_Shell_Command*   shellCommand = Ifx_Oe_Shell_commandListFind(shell, commandLine, &args, &commandList);

    if (shellCommand != NULL_PTR)
    {
        IFX_OE_ASSERT(shellCommand->call != FALSE);

        if (shellCommand->call(args, shellCommand->data, shell->io) != FALSE)
        {
            if (shell->control.sendResultCode != 0)
            {
                Ifx_Oe_Shell_writeResult(shell, Ifx_Oe_Shell_ResultCode_ok);
            }

            result = Ifx_Oe_Shell_ExecuteCode_ok;
        }
        else
        {
            if (shell->control.sendResultCode != 0)
            {
                Ifx_Oe_Shell_writeResult(shell, Ifx_Oe_Shell_ResultCode_nok);
            }
            ...
            result = Ifx_Oe_Shell_ExecuteCode_cmdError;
        }
    }
    else
    {
        if (commandLine[0] != IFX_OE_SHELL_NULL_CHAR)
        {
            if (shell->control.sendResultCode != 0)
            {
                Ifx_Oe_Shell_writeResult(shell, Ifx_Oe_Shell_ResultCode_unknown);
            }
            else if (shell->control.echoError != 0)
            {
            ...
            result = Ifx_Oe_Shell_ExecuteCode_notFound;
        }
        else
        {
            result = Ifx_Oe_Shell_ExecuteCode_ok;
...
void Ifx_Oe_Shell_cmdEscapeProcess(Ifx_Oe_Shell* shell, char EscapeChar1, char EscapeChar2)
{
    Ifx_Oe_Shell_CmdLine* Cmd    = NULL_PTR; /* Command line editing state */
    ...
    IFX_OE_ASSERT(cmdStr != NULL_PTR);
    IFX_OE_ASSERT(Cmd->cursor < IFX_CFG_OE_SHELL_CMD_LINE_SIZE);
    IFX_OE_ASSERT(Cmd->length < IFX_CFG_OE_SHELL_CMD_LINE_SIZE);
    ...
    case 'A':                  /* Up arrow */
        ...
        if (Cmd->historyItem == IFX_OE_SHELL_CMD_HISTORY_NO_ITEM)
        {
        ...
        {
            if (Cmd->historyItem < (IFX_CFG_OE_SHELL_CMD_HISTORY_SIZE - 1))
            {
        ...
        strncpy(cmdStr, shell->cmdHistory[Cmd->historyItem], IFX_CFG_OE_SHELL_CMD_LINE_SIZE);
    ...
    case 'B':                                        /* Down arrow */
        ...
        if ((Cmd->historyItem == IFX_OE_SHELL_CMD_HISTORY_NO_ITEM) || (Cmd->historyItem == 0))
        {
            ...
            Cmd->cursor      = 0;
            Cmd->historyItem = IFX_OE_SHELL_CMD_HISTORY_NO_ITEM; /* Ensure we are not using list */
        }
        else
        {
    ...
    if (EscapeChar2 == '~')
    {
        ...
        case '1':              /* HOME - move to start of buffer */
            ...
            if (Cmd->cursor > 0)
            {
                IFX_OE_SHELL_WRITE_BACKSPACES(Cmd->cursor) Cmd->cursor = 0;
            }
        ...
        case '2':              /* INSERT - insert blank character at cursor and move all remaining characters right one */
            ...
            if ((Cmd->cursor < Cmd->length) && (Cmd->length < (IFX_CFG_OE_SHELL_CMD_LINE_SIZE - 1)))
            {
                /* Update on screen */
                if (shell->control.echo != FALSE)
                {
                    ...
                    IFX_OE_SHELL_WRITE_BACKSPACES((Cmd->length + 1) - Cmd->cursor)
                }
                ...
                cmdStr[Cmd->length + 1] = IFX_OE_SHELL_NULL_CHAR; /* Terminate string at end of longer string */
        ...
        case '3':              /* DELETE - delete character to right and move all remaining characters left one */
            ...
            if (Cmd->cursor < Cmd->length)
            {
                ...
                cmdStr[Cmd->length - 1] = IFX_OE_SHELL_NULL_CHAR; /* Terminate string at end of shorter string */
...
void Ifx_Oe_Shell_disable(Ifx_Oe_Shell* shell)
{
