...
/*-----------------------------------------------------Includes------------------------------------------------------*/
/*********************************************************************************************************************/
#include "Digital_Acquisition_Actuation/Digital_Acquisition/Slk_DAcq_Fuc3.h"
...
IfxCcu6_Icu_Channel g_icuChannelMission;           /* CCU6 ICU channel handle                                              */
float32 g_Ccu6CaputreFrequency = 0.0;           /* Global variable which stores the CCU6 capture frequency              */
...
/*------------------------------------------------Function Prototypes------------------------------------------------*/
/*********************************************************************************************************************/
void initCcu6IcuMissionDAcqFuc3(void);
void initGpt12MonitorDAcqFuc3(void);
void generatePwmFuc3(void);
/* Macro to define the Interrupt Service Routine */
IFX_INTERRUPT(isrGpt12T2MonitorDAcqFuc3, CPU1_RUNNING_TASK, ISR_PRIORITY_GPT12_T2_INT_QACQ_FUC3);
IFX_INTERRUPT(istGpt12T3MonitorDAcqFuc3, CPU1_RUNNING_TASK, ISR_PRIORITY_GPT12_T3_INT_QACQ_FUC3);

IFX_INTERRUPT(isrCcu6PwmMissionEdgeDetectionDAcqFuc3, CPU1_RUNNING_TASK, ISR_PRIORITY_CCU6_RE_DACQ_FUC3);
IFX_INTERRUPT(isrCcu6PeriodMatchDAcqFuc3, CPU1_RUNNING_TASK, ISR_PRIORITY_CCU6_PM_DACQ_FUC3);

/*********************************************************************************************************************/
/*---------------------------------------------Function Implementations----------------------------------------------*/
/*********************************************************************************************************************/
/* Interrupt Service Routine of the CCU6, which gets triggered by a rising edge of the input port pin of timer T12 */
void isrCcu6PwmMissionEdgeDetectionDAcqFuc3(void)
{
    ...
    if (g_icuChannelMission.ccu6->IS.B.ICC60R)
    {
        ...
        risingEdgeCntVal = IfxCcu6_getCaptureRegisterValue(g_icuChannelMission.ccu6, g_icuChannelMission.channelId);
    }

    /* Check on the occurrence of the falling edge */
    if (g_icuChannelMission.ccu6->IS.B.ICC60F)
    {
        /* Reset interrupt status flag */
        ...
        fallingEdgeCntVal = IfxCcu6_getCaptureShadowRegisterValue(g_icuChannelMission.ccu6, g_icuChannelMission.channelId);
    }
    ...
    g_digitalAcqActStatus.pwmFrequency[0] = g_Ccu6CaputreFrequency / (float32)finalFreqCntVal;
...
void isrCcu6PeriodMatchDAcqFuc3(void)
{
...
void isrGpt12T2MonitorDAcqFuc3(void)
{
    ...
    g_digitalAcqActStatus.pwmFrequency[1] = IfxGpt12_T3_getFrequency(&MODULE_GPT120) / (float32)finalCntVal;
...
void istGpt12T3MonitorDAcqFuc3(void)
{
 ...
 *  */
void checkPlausibilityCcu6Gpt12Fuc3(void)
{
...
void initCcu6IcuMissionDAcqFuc3(void)
{
    ...
    g_Ccu6CaputreFrequency = ccu6Handle.captureFrequency;
    ...
    ccu6ChannelConfig.interrupt1.priority = ISR_PRIORITY_CCU6_RE_DACQ_FUC3;/*Configure the int priority*/
    ...
    ccu6ChannelConfig.interrupt2.priority = ISR_PRIORITY_CCU6_PM_DACQ_FUC3;/*Configure the int priority*/
    /* Select the type of interrupt service */
    ...
    {
        &CCU6_INPUT_MISSION_PIN_FUC3,    /* CC60In pin                   */
    ...
    IfxCcu6_Icu_initChannel(&g_icuChannelMission, &ccu6ChannelConfig);         /* Initialize the channel                  */

    /* Start the timer T12 by setting the run bit T12R with the associated bit T12RS */
    IfxCcu6_Icu_startCapture(&g_icuChannelMission);
}
...
void initGpt12MonitorDAcqFuc3(void)
{
    ...
    IfxSrc_init(src, IfxSrc_Tos_cpu1, ISR_PRIORITY_GPT12_T2_INT_QACQ_FUC3);
    IfxSrc_enable(src);

    /* Get source pointer of timer T3, initialize and enable */
    src = IfxGpt12_T3_getSrc(&MODULE_GPT120);
    IfxSrc_init(src, IfxSrc_Tos_cpu1, ISR_PRIORITY_GPT12_T3_INT_QACQ_FUC3);
    IfxSrc_enable(src);
