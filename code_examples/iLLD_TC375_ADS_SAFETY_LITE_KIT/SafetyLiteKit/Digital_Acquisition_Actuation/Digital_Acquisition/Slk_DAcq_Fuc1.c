...
#include "Digital_Acquisition_Actuation/Digital_Acquisition/Slk_DAcq_Fuc1.h"
...
IFX_EXTERN IfxGtm_Tom_Pwm_Driver            g_tomPwmMonitorHandler; /* Handler for TOM PWM configuration */
...
/*------------------------------------------------Function Prototypes------------------------------------------------*/
/*********************************************************************************************************************/
void initMissionTimFuc1(void);
void initMonitorTimFuc1(void);
void generateMissionPWM(void);
void generateMonitorPWM(void);

/* Declarations of the ISR functions */
IFX_INTERRUPT(isrGtmTimPwmMissionDAcqFuc1, CPU1_RUNNING_TASK, ISR_PRIORITY_GTM_TIM_MISSION_DACQ_FUC1);
IFX_INTERRUPT(isrGtmTimPwmMonitorDAcqFuc1, CPU1_RUNNING_TASK, ISR_PRIORITY_GTM_TIM_MONITOR_DACQ_FUC1);

/*********************************************************************************************************************/
/*---------------------------------------------Function Implementations----------------------------------------------*/
/*********************************************************************************************************************/
/* Mission TIM ISR handler */
void isrGtmTimPwmMissionDAcqFuc1(void)
{
    ...
    else
    {
        ...
        /* Execute SM:GTM_TIM_REDUNDANCY by calling checkRedundancyGtmTimHwDiversity only from mission TIM channel */
        checkRedundancyGtmTimHwDiversity();
    }
}

/* Monitor on TIM ISR handler */
void isrGtmTimPwmMonitorDAcqFuc1(void)
{
    IfxCpu_enableInterrupts();
    ...
    else
    {
        ...
        g_digitalAcqActStatus.pwmDutyCylce[1] = IfxGtm_Tim_In_getDutyPercent(&g_timPwmMonitorHandler,
                &g_timPwmMonitorHandler.dataCoherent); /* Get the duty cycle     */
 ...
 * */
void checkRedundancyGtmTimHwDiversity(void)
{
...
void initMissionTimFuc1()
{
    ...
    timPwmMeasMissionConfig.isrPriority = ISR_PRIORITY_GTM_TIM_MISSION_DACQ_FUC1;
...
void initMonitorTimFuc1()
{
    ...
    timPwmMeasMonitorConfig.isrPriority = ISR_PRIORITY_GTM_TIM_MONITOR_DACQ_FUC1;
...
void generateMissionPWM(void)
{
    ...
    pwmOutputSignalConfig.tomChannel = GTM_TOM_SAFE_PIN.channel; /* select channel */
...
void generateMonitorPWM(void)
{
    ...
    pwmOutputSignalConfig.tom = GTM_TOM_SAFE_PIN2.tom; /* Select the TOM */
    pwmOutputSignalConfig.tomChannel = GTM_TOM_SAFE_PIN2.channel; /* select the channel */
    pwmOutputSignalConfig.pin.outputPin = &GTM_TOM_SAFE_PIN2; /* Set the port pin as output */
    pwmOutputSignalConfig.clock = DA_TOM_CLOCK_SOURCE; /* Select the clock source*/
    pwmOutputSignalConfig.synchronousUpdateEnabled = TRUE; /* Enable synchronous update */
    pwmOutputSignalConfig.period = TOM_PWM_PERIOD; /* Set the timer period */
    pwmOutputSignalConfig.dutyCycle = (uint32) (TOM_DUTY_CYCLE * 0.01 * pwmOutputSignalConfig.period);
    ...
    /* Initialize the GTM TOM */
    boolean success = IfxGtm_Tom_Pwm_init(&g_tomPwmMonitorHandler, &pwmOutputSignalConfig);
    if (!success)
    {
        softwareCoreAlarmTriggerSMU(SOFT_SMU_ALM_DIGITAL_ACQ_ACT);
        return;
    ...
    /* Start the PWM */
    IfxGtm_Tom_Pwm_start(&g_tomPwmMonitorHandler, TRUE);
}
/*
 * initial Digital Acquisition FUC1
 * */
void initDAcqFuc1(void)
{
    ...
    /* Part 5: Configure GTM TOM for the dummy output PWM signal which is measured by the TOMs */
    generateMonitorPWM();
