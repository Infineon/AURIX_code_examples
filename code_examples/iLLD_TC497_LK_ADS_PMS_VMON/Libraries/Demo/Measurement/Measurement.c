...
float32 Vddexths;
float32 Vddexths_min;
float32 Vddexths_max;
...
void measurement_init(void)
{
    ...
    IfxPmsEvr_enableVoltageRail(&MODULE_PMS, IfxPmsEvr_PrimaryMonitorVoltageSource_vddexths);
    ...
    driverConfig.cluster = (IfxEgtm_Cluster)MEASUREMENT_MEASURE.cluster;
...
IFX_INTERRUPT(ISR_measurement_call, CPU_WHICH_SERVICE_MEASUREMENT, ISR_PRIORITY_MEASUREMENT)
{
    ...
    voltage = IfxPmsEvr_getPrimaryMonitorResult(&MODULE_PMS, IfxPmsEvr_PrimaryMonitorVoltageSource_vddexths);
    Vddexths = (float)voltage*0.00283f;
    if(Vddexths > Vddexths_max)
        Vddexths_max = Vddexths;
    if(Vddexths < Vddexths_min)
        Vddexths_min = Vddexths;
