...
typedef struct
{
    IfxEgtm_Tim_In *inputChannel;       /**< \brief Pointer to input channel handle */
...
} IfxEgtm_Tim_In_BitCompressionInput;
 ...
 * \param[in]    numOfInputChannels Number of entries in inputSetupArray. Range: 1 to \ref IFXEGTM_NUM_TIM_CHANNELS.
 *
 * \retval None
 */
IFX_INLINE void IfxEgtm_Tim_In_setBitCompressionInputs(IfxEgtm_Tim_In *driver, IfxEgtm_Tim_In_BitCompressionInput *inputSetupArray, uint8 numOfInputChannels);
 ...
 */
IFX_INLINE uint8 IfxEgtm_Tim_In_getBitCompressionData(IfxEgtm_Tim_In *driver);
 ...
 * \retval None
 */
IFX_INLINE void IfxEgtm_Tim_In_disableChannel(IfxEgtm_Tim_In *driver);
 ...
 * \retval None
 */
IFX_INLINE void IfxEgtm_Tim_In_enableChannel(IfxEgtm_Tim_In *driver);
 ...
 * \retval None
 */
IFX_INLINE void IfxEgtm_Tim_In_updateGpr1(IfxEgtm_Tim_In *driver, uint32 value);
...
IFX_INLINE void IfxEgtm_Tim_In_setBitCompressionInputs(IfxEgtm_Tim_In *driver, IfxEgtm_Tim_In_BitCompressionInput *inputSetupArray, uint8 numOfInputChannels)
{
    ...
    for (index = 0u; index < numOfInputChannels; index++)
    {
        ...
        IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, ((uint32)inputSetupArray[index].inputChannel->channelIndex < IFXEGTM_NUM_TIM_CHANNELS));

        /* Build per-channel CNTS mask: bit ch (rising) and bit m+ch (falling) */
        mask |= (uint32)(((1u << IFXEGTM_NUM_TIM_CHANNELS) | 1u) << inputSetupArray[index].inputChannel->channelIndex);
        ...
        {
            value |= (uint32)((1u << IFXEGTM_NUM_TIM_CHANNELS) << inputSetupArray[index].inputChannel->channelIndex);
        }
    }

    driver->channel->CTRL.B.TIM_MODE = (uint8)IfxEgtm_Tim_Mode_bitCompression;
