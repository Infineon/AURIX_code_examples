/**
 * \file IfxGtm_Tim_In.h
 * \brief GTM IN details
 * \ingroup IfxLld_Gtm
 *
 * \version iLLD-TC4-v2.6.0
 * \copyright Copyright (c) 2026 Infineon Technologies AG. All rights reserved.
 *
 *
 *
 *                                 IMPORTANT NOTICE
 *
 * Infineon Technologies AG (Infineon) licenses this file to you under the
 * Infineon Automotive SW Lab License v2025-01 (IFASLL). You may not use
 * this file except in compliance with IFASLL.
 *
 * The full license text is contained in IFASLL202501.pdf delivered with this SW.
 * Unless required by applicable law or agreed to in writing, software distributed
 * under this license is distributed "AS IS" without any warranty or liability of any
 * kind and Infineon hereby expressly disclaims any warranties or representations,
 * whether express, implied, statutory or otherwise, including but not limited to
 * warranties of workmanship, merchantability, fitness for a particular purpose,
 * defects in the licensed items, or non-infringement of third parties'
 * intellectual property rights. See the full license text for the specific
 * language governing permissions and limitations under IFASLL.
 *
 *
 * \defgroup IfxLld_Gtm_Tim_In_Usage How to use the TIM Input Interface layer?
 * \ingroup IfxLld_Gtm_Tim_In
 *
 * \section IfxLld_Gtm_Tim_In_Overview Overview
 *
 * The TIM Input interface layer is used for usecases such as PWM measurement. It uses the TIM submodule of GTM to perform this measurement.
 *
 * \section IfxLld_Gtm_Tim_In_Code Coding Example
 *
 * Below is an example that describes how to capture signals from a pin and measure the frequency and duty cycle of the signal.
 *
 * \subsection IfxLld_Gtm_Tim_In_Step1 Step 1: Enable GTM and its clocks
 * Please enable the GTM module and enable the required clocks for the TIM submodule.
 *
 * \subsection IfxLld_Gtm_Tim_In_Step2 Step 2: Instantiate variables
 *
 *     \code
 *     // Include interface
 *     #include "Gtm/Tim/In/IfxGtm_Tim_In.h"
 *
 *     // Instantiate variables
 *     IfxGtm_Tim_In          timIn;          // Driver handle
 *     IfxGtm_Tim_In_Config   timInConfig;    // Configuration structure
 *     \endcode
 *
 * \subsection IfxLld_Gtm_Tim_In_Step3 Step 3: [OPTIONAL] Define interrupt handler
 *
 *     \code
 *     volatile unsigned timIsrCounter = 0;
 *
 *     IFX_INTERRUPT(TimDemo_ISR, 0, 1)
 *     {
 *         // Update values on interrupt
 *         IfxGtm_Tim_In_onIsr(&timIn);
 *         timIsrCounter += 1;
 *     }
 *     \endcode
 *
 * \subsection IfxLld_Gtm_Tim_In_Step4 Step 4: Initalize TIM
 *
 *     \code
 *     // Initialize the Config Structure
 *     IfxGtm_Tim_In_initConfig(&timInConfig, &MODULE_GTM);
 *
 *     // Configure the Channel used
 *     timInConfig.cluster                      = IfxGtm_Tim_0;
 *     timInConfig.channelIndex                 = IfxGtm_Tim_Ch_0;
 *     // Configure Interrupt parameters
 *     timInConfig.irqMode                      = IfxGtm_IrqMode_pulseNotify;
 *     timInConfig.isrProvider                  = IfxSrc_Tos_cpu0;
 *     timInConfig.isrPriority                  = 1;
 *     // Give an interrupt on new value
 *     timInConfig.capture.irqOnNewVal          = TRUE;
 *     // Configure the Input
 *     timInConfig.inputSrc                     = IfxGtm_Tim_In_Src_channelCurrentOrPrevious;
 *     timInConfig.inputPin              		= &IfxGtm_TIM0_0_P02_0_IN;
 *     timInConfig.inputPinMode          		= IfxPort_InputMode_pullDown;
 *
 *     // Initialize the TIM Channel now
 *     IfxGtm_Tim_In_init(&timIn, &timInConfig);
 *
 *     \endcode
 *
 * Now, whenever a new measurement is captured, the function IfxGtm_Tim_In_onIsr is called. This will update the value of frequency and duty in handle
 *
 * \subsection IfxLld_Gtm_Tim_In_Step5 Step 5: Read measurement
 *
 *     \code
 *     // Check if data is coherent
 *     if (timIn.dataCoherent == TRUE)
 *     {
 *         timIn.dataCoherent = FALSE;
 *
 *         // Get measurements
 *         float32 measuredPwmFrequency    = 1.0f / IfxGtm_Tim_In_getPeriodSecond(&timIn);
 *         float32 measuredDutyPercent     = IfxGtm_Tim_In_getDutyPercent(&timIn, &dataCoherent);
 *     }
 *     \endcode
 *
 * \defgroup IfxLld_Gtm_Tim_In TIM Input Interface
 * \ingroup IfxLld_Gtm_Tim
 * \defgroup IfxLld_Gtm_Tim_In_DataStructures Data Structures
 * \ingroup IfxLld_Gtm_Tim_In
 * \defgroup IfxLld_Gtm_Tim_In_Enumerations Enumerations
 * \ingroup IfxLld_Gtm_Tim_In
 * \defgroup IfxLld_Gtm_Tim_In_Funtions Funtions
 * \ingroup IfxLld_Gtm_Tim_In
 */

#ifndef IFXGTM_TIM_IN_H
#define IFXGTM_TIM_IN_H 1

/******************************************************************************/
/*----------------------------------Includes----------------------------------*/
/******************************************************************************/

#include "_PinMap/IfxGtm_PinMap.h"
#include "Gtm/Std/IfxGtm_Tim.h"
#include "Gtm/Std/IfxGtm_Cmu.h"
#if !defined(IFX_ILLD_PPU_USAGE)
#include "Cpu/Std/IfxCpu.h"
#endif
#include "_Utilities/Ifx_Assert.h"

/******************************************************************************/
/*--------------------------------Enumerations--------------------------------*/
/******************************************************************************/

/** \addtogroup IfxLld_Gtm_Tim_In_Enumerations
 * \{ */
/** \brief Config Filter Mode.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.FLT_CTR_RE and Ifx_GTM_TIM_CH_CTRL.B.FLT_CTR_FE
 */
typedef enum
{
    IfxGtm_Tim_In_ConfigFilterMode_immediateEdgePropagation,      /**< \brief Immediate edge propagation mode */
    IfxGtm_Tim_In_ConfigFilterMode_individualDeglitchTimeUpDown,  /**< \brief Individual deglitch time mode (Up Down) */
    IfxGtm_Tim_In_ConfigFilterMode_individualDeglitchTimeHold,    /**< \brief Individual deglitch time mode (Hold) */
    IfxGtm_Tim_In_ConfigFilterMode_none                           /**< \brief No filter */
} IfxGtm_Tim_In_ConfigFilterMode;

/** \brief Input Source for the channel
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.CICTRL
 */
typedef enum
{
    IfxGtm_Tim_In_SrcForTimIn_currentChannel  = 0, /**< \brief Use the input from the current channel */
    IfxGtm_Tim_In_SrcForTimIn_previousChannel = 1  /**< \brief Use the input from the previous channel */
} IfxGtm_Tim_In_SrcForTimIn;

/** \} */

/** \brief Input Source common for both LUT enabled and bypassed case
 */
typedef enum
{
    IfxGtm_Tim_In_Src_channelCurrentOrPrevious = 0,  /**< \brief Use the input signal for Current/Previous channel */
    IfxGtm_Tim_In_Src_auxiliary                = 1,  /**< \brief Use the auxiliary input */
    IfxGtm_Tim_In_Src_software                 = 2   /**< \brief Use the Software input fed at VALx */
} IfxGtm_Tim_In_Src;

/** \brief Input Source for LUT 2nd channel
 */
typedef enum
{
    IfxGtm_Tim_In_SrcForLutIn2_external_capture              = 0,  /**< \brief Use the External Capture input */
    IfxGtm_Tim_In_SrcForLutIn2_outputSignal_previousChannel  = 1,  /**< \brief Use the Input Signal from Previous Channel */
    IfxGtm_Tim_In_SrcForLutIn2_outputSignal_tssmMode         = 2   /**< \brief Use the Output signal in the TSSM mode */
} IfxGtm_Tim_In_SrcForLutIn2;

/** \brief Active edge for measurement.
 * Definition in Ifx_GTM_TIM_CH_CTRL.B.DSL
 */
typedef enum
{
    IfxGtm_Tim_In_ActiveEdge_falling,  /**< \brief Use falling edge as active edge */
    IfxGtm_Tim_In_ActiveEdge_raising,  /**< \brief Use raising edge as active edge */
    IfxGtm_Tim_In_ActiveEdge_both      /**< \brief Use both edge as active edge */
} IfxGtm_Tim_In_ActiveEdge;

/******************************************************************************/
/*-----------------------------Data Structures--------------------------------*/
/******************************************************************************/

/** \addtogroup IfxLld_Gtm_Tim_In_DataStructures
 * \{ */
/** \brief Configuration structure for TIM capture
 */
typedef struct
{
    boolean                  irqOnNewVal;             /**< \brief If TRUE, the interrupt on new value is enabled */
    boolean                  irqOnCntOverflow;        /**< \brief If TRUE, the interrupt on CNT overflow is enabled */
    boolean                  irqOnEcntOverflow;       /**< \brief If TRUE, the interrupt on ECNT (Edge counter) overflow is enabled */
    boolean                  irqOnDatalost;           /**< \brief If TRUE, the interrupt on data lost (GPR0, GPR1) is enabled */
    IfxGtm_Cmu_Clk           clock;                   /**< \brief Timer input clock */
    Ifx_Pwm_Mode             mode;                    /**< \brief PWM mode, only Ifx_Pwm_Mode_leftAligned and Ifx_Pwm_Mode_righAligned are supported */
    IfxGtm_Tim_In_ActiveEdge activeEdge;              /**< \brief Active edge to be selected as falling, raising or both */
    uint32                   gateCount;               /**< \brief Shadow count value. Range: 0 to 0xFFFFFF */
} IfxGtm_Tim_In_ConfigCapture;

/** \brief Configuration structure for TIM filter
 */
typedef struct
{
    IfxGtm_Cmu_Tim_Filter_Clk      clock;                       /**< \brief Timeout clock */
    IfxGtm_Tim_In_ConfigFilterMode risingEdgeMode;              /**< \brief Filter mode for rising edge */
    IfxGtm_Tim_In_ConfigFilterMode fallingEdgeMode;             /**< \brief Filter mode for falling edge */
    float32                        risingEdgeFilterTime;        /**< \brief Rising edge filter time  in second */
    float32                        fallingEdgeFilterTime;       /**< \brief Falling edge filter time in second */
    boolean                        irqOnGlitch;                 /**< \brief If TRUE, the interrupt on glitch is enabled */
} IfxGtm_Tim_In_ConfigFilter;

/** \brief Configuration structure for TIM timeout
 */
typedef struct
{
    IfxGtm_Cmu_Clk clock;              /**< \brief Timeout clock */
    float32        timeout;            /**< \brief Timeout in second. Value of 0 disable the timeout functionality see 27.10.3 Timeout Detection Unit (TDU) */
    boolean        irqOnTimeout;       /**< \brief If TRUE, the interrupt on timeout is enabled */
} IfxGtm_Tim_In_ConfigTimeout;

/** \} */

/** \addtogroup IfxLld_Gtm_Tim_In_DataStructures
 * \{ */
/** \brief Driver Handle
 */
typedef struct
{
    Ifx_GTM_CLS_TIM_CH *channel;                     /**< \brief TIM channel used */
    uint32              periodTick;                  /**< \brief Period value in clock ticks. Range: 0 to 0xFFFFFF */
    uint32              pulseLengthTick;             /**< \brief Duty value in clock ticks. Range: 0 to 0xFFFFFF */
    boolean             dataCoherent;                /**< \brief TRUE, if the duty and period values are measured from the same period */
    boolean             overflowCnt;                 /**< \brief TRUE if the last measurement show an overflow in CNT */
    boolean             newData;                     /**< \brief TRUE when values are updated, and  if none of the counter CNT, CNTS have overflowed */
    boolean             dataLost;                    /**< \brief TRUE if data are lost */
    uint32              edgeCounterUpper;            /**< \brief upper part of the edge counter. Range: 0 to 0xFFFF */
    boolean             glitch;                      /**< \brief TRUE if glitch is detected */
    float32             captureClockFrequency;       /**< \brief Capture clock frequency in Hz */
    IfxGtm_Tim          clsIndex;                    /**< \brief Index of the CLS module being used. */
    IfxGtm_Tim_Ch       channelIndex;                /**< \brief Index of the TIM channel being used. */
    uint16              edgeCount;                   /**< \brief number of edges counted. Range: 0 to 0xFFFF */
    boolean             softwareInputSet;            /**< \brief flag to check if the software input is provided */
    Ifx_GTM_CLS_TIM    *tim;                         /**< \brief Pointer to TIM module */
} IfxGtm_Tim_In;

/** \brief Configuration structure for TIM input capture
 */
typedef struct
{
    Ifx_GTM                    *gtm;                /**< \brief GTM used */
    IfxGtm_Tim                  cluster;            /**< \brief CLS index */
    IfxGtm_Tim_Ch               channelIndex;       /**< \brief Channel index */
    IfxGtm_IrqMode              irqMode;            /**< \brief Interrupt mode for the new value available */
    IfxSrc_Tos                  isrProvider;        /**< \brief Interrupt service provider for the timer interrupt */
    Ifx_Priority                isrPriority;        /**< \brief Set the interrupt priority for new value available. If 0, no interrupt will be generated */
    IfxGtm_Tim_In_ConfigCapture capture;            /**< \brief Capture configuration */
    IfxGtm_Tim_In_ConfigFilter  filter;             /**< \brief Filter configuration */
    IfxGtm_Tim_In_ConfigTimeout timeout;            /**< \brief Timeout configuration */
    IfxSrc_VmId                 vmId;               /**< \brief Virtual machine interrupt service provider */
    IfxGtm_Tim_Mode             mode;               /**< \brief TIM channel Mode */
    IfxGtm_Tim_In_Src           inputSrc;           /**< \brief Input Source for LUT bypassed case plus LUT_IN0 for LUT case */
    IfxGtm_Tim_In_SrcForLutIn2  input2ForLUT;       /**< \brief selected input for LUT 2 channel */
    IfxGtm_Tim_In_SrcForTimIn   srcForTimIn;        /**< \brief CICTRL selection to decide current or previous channel */
    boolean                     useLUT;             /**< \brief Input to check if the LUT is to be enabled or not */
    IfxGtm_Tim_TinMap          *inputPin;           /**< \brief Input selection for the channel */
    IfxPort_InputMode           inputPinMode;       /**< \brief Input pin mode */
    uint8                       lookupTableValue;   /**< \brief Lookup Table Content */
} IfxGtm_Tim_In_Config;

/** \brief Bit compression input setup for TIM_IN channel
 */
typedef struct
{
    IfxGtm_Tim_In *inputChannel;       /**< \brief Pointer to input channel handle */
    boolean        enableRisingEdge;   /**< \brief TRUE to enable rising edge from the input channel */
    boolean        enableFallingEdge;  /**< \brief TRUE to enable falling edge from the input channel */
} IfxGtm_Tim_In_BitCompressionInput;

/** \} */

/** \addtogroup IfxLld_Gtm_Tim_In_Funtions
 * \{ */

/******************************************************************************/
/*-------------------------Inline Function Prototypes-------------------------*/
/******************************************************************************/

/** \brief Clear the new data flag.
 *
 * \param[inout] driver TIM Input object.
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_clearNewData(IfxGtm_Tim_In *driver);

/** \brief return the dutycycle in percent.
 *
 * \param[in] driver TIM Input object.
 * \param[out] dataCoherent If true, the duty cycle has been calculated with coherent values for the period and duty, else the period and duty value are from 2 adjacent periods.
 *
 * \retval float32 Duty.
 */
IFX_INLINE float32 IfxGtm_Tim_In_getDutyPercent(IfxGtm_Tim_In *driver, boolean *dataCoherent);

/** \brief Return the period in second.
 *
 * \param[in] driver TIM Input object.
 *
 * \retval float32 Period value in seconds.
 */
IFX_INLINE float32 IfxGtm_Tim_In_getPeriodSecond(IfxGtm_Tim_In *driver);

/** \brief Return the period value in tick.
 *
 * \param[in] driver TIM Input object.
 *
 * \retval sint32 Period value in ticks.
 */
IFX_INLINE sint32 IfxGtm_Tim_In_getPeriodTicks(IfxGtm_Tim_In *driver);

/** \brief Return the pulse length value in tick.
 *
 * \param[in] driver TIM Input object.
 *
 * \retval sint32 Pulse length.
 */
IFX_INLINE sint32 IfxGtm_Tim_In_getPulseLengthTick(IfxGtm_Tim_In *driver);

/** \brief Indicates if data were lost.
 *
 * \param[in] driver TIM Input object.
 *
 * \retval TRUE if Data is lost FALSE otherwise.
 */
IFX_INLINE boolean IfxGtm_Tim_In_isDataLost(IfxGtm_Tim_In *driver);

/** \brief Indicates if new data are present (new data flag).
 *
 * \param[in] driver TIM Input object.
 *
 * \retval TRUE if New Data FALSE otherwise.
 */
IFX_INLINE boolean IfxGtm_Tim_In_isNewData(IfxGtm_Tim_In *driver);

/** \brief Configures the Software Input
 *
 * \param[inout] driver        TIM Input Object
 * \param[in]    softwareInput Software Input for the LUT
 *                             Range: TRUE: Software input is provided.
 *                                    FALSE: Software input is not provided
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_configureSoftwareInput(IfxGtm_Tim_In *driver, boolean softwareInput);

/** \brief Configures selected TIM input channels for bit-compression mode
 *
 * \param[inout] driver             TIM Input object
 * \param[in]    inputSetupArray    Array of input channel setup entries
 * \param[in]    numOfInputChannels Number of entries in inputSetupArray. Range: 1 to \ref IFXGTM_NUM_TIM_CHANNELS
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_setBitCompressionInputs(IfxGtm_Tim_In *driver, IfxGtm_Tim_In_BitCompressionInput *inputSetupArray, uint8 numOfInputChannels);

/** \brief Gets the bit-compressed data from the TIM input channel
 *
 * \param[in] driver TIM Input object
 *
 * \retval uint8 Bit-compressed data
 */
IFX_INLINE uint8 IfxGtm_Tim_In_getBitCompressionData(IfxGtm_Tim_In *driver);

/** \brief Disable TIM Channel
 *
 * \param[inout] driver TIM Input object
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_disableChannel(IfxGtm_Tim_In *driver);

/** \brief Enable TIM Channel
 *
 * \param[inout] driver TIM Input object
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_enableChannel(IfxGtm_Tim_In *driver);

/** \brief Updates the GPR1 register for dynamic period changes in gated periodic sampling mode.
 *
 * \param[inout] driver TIM Input object
 * \param[in]    value  New GPR1 value for the shadow counter update. Range: 0 to 0xFFFFFF
 *
 * \retval None
 */
IFX_INLINE void IfxGtm_Tim_In_updateGpr1(IfxGtm_Tim_In *driver, uint32 value);

/******************************************************************************/
/*-------------------------Global Function Prototypes-------------------------*/
/******************************************************************************/

/** \brief Initializes the input capture object.
 *
 * \param[inout] driver TIM Input object.
 * \param[in]    config Configuration structure for the input capture Timer.
 *
 * \retval TRUE on success else FALSE
 */
IFX_EXTERN boolean IfxGtm_Tim_In_init(IfxGtm_Tim_In *driver, const IfxGtm_Tim_In_Config *config);

/** \brief Initializes the configuration structure to default.
 *
 * \param[inout] config Configuration structure for the input capture Timer
 * \param[in]    gtm    Pointer to GTM module.
 *
 * \retval None
 */
IFX_EXTERN void IfxGtm_Tim_In_initConfig(IfxGtm_Tim_In_Config *config, Ifx_GTM *gtm);

/** \brief Will update the  driver state\n.
 * To be called in the interrupt generated by the IfxGtm_Tim_In driver.
 *
 * \param[inout] driver TIM Input object.
 *
 * \retval None
 */
IFX_EXTERN void IfxGtm_Tim_In_onIsr(IfxGtm_Tim_In *driver);

/** \brief Updates the period and duty cycle.
 *
 * \param[inout] driver TIM Input object.
 *
 * \retval None
 */
IFX_EXTERN void IfxGtm_Tim_In_update(IfxGtm_Tim_In *driver);

/** \} */

/******************************************************************************/
/*---------------------Inline Function Implementations------------------------*/
/******************************************************************************/

IFX_INLINE void IfxGtm_Tim_In_clearNewData(IfxGtm_Tim_In *driver)
{
    driver->newData = FALSE;
}


IFX_INLINE float32 IfxGtm_Tim_In_getDutyPercent(IfxGtm_Tim_In *driver, boolean *dataCoherent)
{
    float32 duty;
#if !defined(IFX_ILLD_PPU_USAGE)
    boolean interruptState = IfxCpu_disableInterrupts();
#endif
    duty          = (float32)((float32)driver->pulseLengthTick * (float32)100.0f) / (float32)driver->periodTick;
    *dataCoherent = driver->dataCoherent;
#if !defined(IFX_ILLD_PPU_USAGE)
    IfxCpu_restoreInterrupts(interruptState);
#endif

    return duty;
}


IFX_INLINE float32 IfxGtm_Tim_In_getPeriodSecond(IfxGtm_Tim_In *driver)
{
    return (float32)IfxGtm_Tim_In_getPeriodTicks(driver) / driver->captureClockFrequency;
}


IFX_INLINE sint32 IfxGtm_Tim_In_getPeriodTicks(IfxGtm_Tim_In *driver)
{
    return (sint32)driver->periodTick;
}


IFX_INLINE sint32 IfxGtm_Tim_In_getPulseLengthTick(IfxGtm_Tim_In *driver)
{
    return (sint32)driver->pulseLengthTick;
}


IFX_INLINE boolean IfxGtm_Tim_In_isDataLost(IfxGtm_Tim_In *driver)
{
    return driver->dataLost;
}


IFX_INLINE boolean IfxGtm_Tim_In_isNewData(IfxGtm_Tim_In *driver)
{
    return driver->newData;
}


IFX_INLINE void IfxGtm_Tim_In_configureSoftwareInput(IfxGtm_Tim_In *driver, boolean softwareInput)
{
    IfxGtm_Tim_InputSourceSelect inputSourceSelect;

    if (driver->softwareInputSet == TRUE)
    {
        inputSourceSelect.mode = 2;

        if (softwareInput == TRUE)
        {
            inputSourceSelect.value = 2;
        }
        else
        {
            inputSourceSelect.value = 1;
        }

        IfxGtm_Tim_Ch_setInputSourceSelection(driver->tim, driver->channelIndex, inputSourceSelect);
    }
}


IFX_INLINE void IfxGtm_Tim_In_setBitCompressionInputs(IfxGtm_Tim_In *driver, IfxGtm_Tim_In_BitCompressionInput *inputSetupArray, uint8 numOfInputChannels)
{
    uint32 index;
    uint32 mask  = 0u;
    uint32 value = 0u;

    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, (numOfInputChannels > 0u));

    for (index = 0u; index < numOfInputChannels; index++)
    {
        /* Input channel index must be within TIM channel range */
        IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, ((uint32)inputSetupArray[index].inputChannel->channelIndex < IFXGTM_NUM_TIM_CHANNELS));

        /* Build per-channel CNTS mask: bit ch (rising) and bit m+ch (falling) */
        mask |= (uint32)(((1u << IFXGTM_NUM_TIM_CHANNELS) | 1u) << inputSetupArray[index].inputChannel->channelIndex);

        /* Build CNTS bit selection: bits [0..m-1] rising, bits [m..2m-1] falling */
        if (inputSetupArray[index].enableRisingEdge == TRUE)
        {
            value |= (uint32)(1u << inputSetupArray[index].inputChannel->channelIndex);
        }

        if (inputSetupArray[index].enableFallingEdge == TRUE)
        {
            value |= (uint32)((1u << IFXGTM_NUM_TIM_CHANNELS) << inputSetupArray[index].inputChannel->channelIndex);
        }
    }

    driver->channel->CTRL.B.TIM_MODE = (uint8)IfxGtm_Tim_Mode_bitCompression;
    Ifx__ldmst(&driver->channel->CNTS, mask, value);
}


IFX_INLINE uint8 IfxGtm_Tim_In_getBitCompressionData(IfxGtm_Tim_In *driver)
{
    return (uint8)driver->channel->GPR1.B.GPR1;
}


IFX_INLINE void IfxGtm_Tim_In_disableChannel(IfxGtm_Tim_In *driver)
{
	driver->channel->CTRL.B.TIM_EN = 0U;
}


IFX_INLINE void IfxGtm_Tim_In_enableChannel(IfxGtm_Tim_In *driver)
{
	driver->channel->CTRL.B.TIM_EN = 1U;
}


IFX_INLINE void IfxGtm_Tim_In_updateGpr1(IfxGtm_Tim_In *driver, uint32 value) 
{
    driver->channel->GPR1.B.GPR1 = value;
}


#endif /* IFXEGTM_TIM_IN_H */
