...
IFX_EXTERN unsigned int safetyWatchdogLock;
IFX_EXTERN boolean IfxCpu_acquireMutex(unsigned int *lock);
IFX_EXTERN void IfxCpu_releaseMutex(unsigned int *lock);
...
IFX_INLINE void IfxScuWdt_setSafetyEndinitInline(uint16 password)
{
    ...
    {}
	/* release safety watchdog mutex, that other tasks can access the safety watchdog */
    IfxCpu_releaseMutex(&safetyWatchdogLock);
}
