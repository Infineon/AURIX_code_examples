 ...
 * */
void initEVADCModule(SafetyKitEvadcCfgType *evadcCfg)
{
 ...
 * */
void initEVADCGroups(SafetyKitEvadcCfgType *evadcCfg)
{
...
void initEVADCChannels(SafetyKitEvadcCfgType *evadcCfg)
{
        ...
        const float32 varefExpectedResult, const float32 varefTolerance, const IfxEvadc_RequestSource sourceType)
{
    ...
    sint32 conversionResult = getEvadcChannelResult(&evadcCfg->evadcChannel[varefChId]);

    /* Flush the queue as of VAREF check is only done once per driving cycle. */
    ...
    float32 VAREF = EVADC_RESULT_TO_V(conversionResult);
...
/*---------------------------------------------------------------------------------------------*/
/*                     Additional functions for Safety Kit PCB                                 */
/*---------------------------------------------------------------------------------------------*/
/*
 * Function to calculate the temperature out of the NTC resistance value
 */
float32 convertNtcResistanceToDegrees(float32 R)
{
    float32 temp_res;

    temp_res = 1./( ( log( R / NTC_R25 ) / NTC_BETA_25_85) + (1./NTC_25C_TO_K) );/* K° */
    temp_res -= 273.15; /* [C°] */
        ...
        IfxSmu_Alarm smuSoftwareAlarmDifference)
{
        ...
        (missionChannel < lowerLimit) || (monitorChannel < lowerLimit))
    {
        ...
        softwareCoreAlarmTriggerSMU(smuSoftwareAlarmLimit);
        return FALSE;
    }
    ...
    {
        /* Trigger SMU software alarm */
        softwareCoreAlarmTriggerSMU(smuSoftwareAlarmDifference);
        return FALSE;
    }
