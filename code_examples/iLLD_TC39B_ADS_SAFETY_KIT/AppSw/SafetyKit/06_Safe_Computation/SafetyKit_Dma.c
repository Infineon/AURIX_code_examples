...
uint32 wrongDestination = (uint32)&MODULE_CAN2; /* The address of the memory location of an uninitialized
                                                                          module is used as wrongDestination address */
boolean injectCrcError = FALSE;
boolean injectDestinationError = FALSE;
boolean injectTimestampError = FALSE;
...
/*********************************************************************************************************************/
/*
 * Enable inject crc error flag
 */
void injectCrcErrorDMA(void)
{
    injectCrcError = TRUE;
}

/*
 * Enable inject Destination error flag
 */
void injectDestinationErrorDMA(void)
{
    injectDestinationError = TRUE;
}

/*
 * Enable inject time stamp error flag
 */
void injectTimestampErrorDMA(void)
{
    injectTimestampError = TRUE;
}
...
void isrCH0DMA(void)
{
    ...
    {
        error |= DMA_ADDRESS_CRC_ERROR;
    }

    /* Implement SM:DMA:DATA_CRC */
    if(chn.dma->CH[IfxDma_ChannelId_10].RDCRCR.U != rdcrc)
    {
        error |= DMA_DATA_CRC_ERROR;
    }
...
IFX_INLINE uint32 __crc32b_SafetyKitDMA(uint32 b, uint32 a)
{
...
void accumulateCRC(uint8* sadr, uint8* dadr, uint32 transfers, uint32 moves, uint32 chdw_bytes, uint32 accumulate)
{
    ...
    for (i = 0; i < transfers*moves; i++)
    {
        ...
        else if (chdw_bytes == 2)
        {
            ...
            brd = __crc32b_SafetyKitDMA(brd, a);
...
void initAndRunDmaTransaction(void)
{
    ...
    {
        transactionStartTimeStamp = transactionStartTimeStamp - 5;   /* corrupt time stamp current value */
     /* transactionStartTimeStamp = transactionStartTimeStamp + 5; *//* corrupt time stamp current value */
