...
#define REF                 IfxGtm_TOM1_0_TOUT77_P15_6_OUT                  /* Port pin driven by the PWM signal    */
...
#define MON                 IfxGtm_TOM0_0_TOUT76_P15_5_OUT                  /* Port pin driven by the PWM signal    */
...
{
    IfxPort_setPinLow(LED_D107);                                           /* Switch ON the LED D107                */
    IfxSmu_enableClearAlarmStatus(&MODULE_SMU);                            /* Enable clearing of alarm status       */
    IfxSmu_clearAlarm(&MODULE_SMU, IfxSmu_Alarm_IomPinMismatchIndication); /* Clear the SMU flag related to the IOM */
}
...
void initSmu(void)
{
    ...
    /* Unlock the SMU module to allow its configuration */
    IfxSmu_unlock(&MODULE_SMU);

    /* Allow bit configuration by disabling the safety EndInit protection */
    IfxScuWdt_clearSafetyEndinitInline(IfxScuWdt_getSafetyWatchdogPasswordInline());

    /* Enable the 'Interrupt Generation Configuration Set 0' */
    MODULE_SMU.AGC.U = 0x1;

    /* Re-enable the safety EndInit protection */
    ...
    /* Re-lock the SMU after configuration */
...
void initIOM(void)
{
    ...
    IfxPort_setPinHigh(LED_D107);                                   /* Switch OFF the LED (active low)              */
