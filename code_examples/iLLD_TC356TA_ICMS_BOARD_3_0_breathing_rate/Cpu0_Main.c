 ...
 * IN THE SOFTWARE.
 *********************************************************************************************************************/
 /*\title ICMS RADAR point cloud generation and breathing rate calculation
 * \abstract This software project for the ICMS board v3.0 (AURIX TC356 and BGT60 ATR24AIP) demonstrates the main radar signal processing steps to generate point cloud data from raw data recorded by the MMIC and to calculate the breathing rate of a detected target.
 * \description This software project is intended as a starting point for the application of in cabin monitoring (ICMS) on the ICMS board v3.0. It demonstrates the main processing steps that need to be performed on AURIX TC356 signal processing unit (SPU) and Tricore CPUs to generate point cloud data from raw data recorded by the BGT60 ATR24AIP MMIC and to calculate the breathing rate of a detected target.
 *              
 *              Basic understanding of FMCW radar range-doppler signal processing is presumed to follow this code example.
 *
 * \name iLLD_TC356TA_ICMS_BOARD_3_0_breathing_rate
 * \version V1.0.0
 * \board ICMS board v3.0, ICMS board v3.0, TC35xTA_A-Step
 * \keywords ICMS, RADAR, POINT CLOUD, BREATHING RATE
...
IFX_INLINE void IfxRadarStateMachine()
{
    ...
    switch (rdr->rState)
    {
        ...
        case Radar_State_processing:
            ...
            /*perform clutter removal, direction of arrival (capon beamforming), detection and breathing rate processing*/
            ...
            { /* jump to loop when last frame of micro doppler frame has been recorded */
                ...
                rdr->clockcountsICMS.detections = g_perfCounts.clock.counter;
                 ...
                 /* microdoppler beamforming for each target*/
                IfxCpu_resetAndStartCounters(IfxCpu_CounterMode_normal);
                Beamforming();
                g_perfCounts = IfxCpu_stopCounters();
                rdr->clockcountsICMS.beamforming = g_perfCounts.clock.counter;

                /*breathing rate detection*/
                IfxCpu_resetAndStartCounters(IfxCpu_CounterMode_normal);
                phaseExtraction();
                subtract_mean();
                IfxSpu_reloadConfig(rdr->spu0, 14);
                while(rdr->spuOk < 1); /* wait for SPU to finish processing */
                rdr->spuOk=0; /*reset spuOk*/
                calculate_respiration_rate();
                g_perfCounts = IfxCpu_stopCounters();
                rdr->clockcountsICMS.breathingrate = g_perfCounts.clock.counter;
                __debug();
            }
            rdr->rState = Radar_State_ready;
            break;
