...
/*This SPU config looks for local maxima and applies absolute threshold along range dimension */

IfxSpu_ConfigSlot *IfxSpu_cfg11_rThreshLM(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    ...
    cfg->input.rm.innerLoopRepeat     = RADAR_NUM_DTHETA;
    cfg->input.rm.outerLoopRepeat     = 1;

    cfg->input.rm.binLoopOffset       = bin_size_bytes*RADAR_NUM_DTHETA;
    cfg->input.rm.innerLoopOffset     = bin_size_bytes;
   ...
   IfxSpu_setupLocalMax(slot, &cfg->localMax); /* disabled with passing NULL_PTR */
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        pcfg0->cfar.mode= IfxSpu_CFAR_Mode_localMaxOffline;
        pcfg0->cfar.baseAddress = IFX_OFFSETOF(SPU_Rmem_Map_t, CFARLMRMap[0]);
        ...
        pcfg0->cfarCA.windowCellsExponent =2; /* corresponds to 2^2=4 cells*/
        pcfg0->cfarCA.beta = 0x0C00;/*corresponding to a threshold in log2 domain of 6.*/
        pcfg0->cfarCA.cashSubWindowExponent = 2; /* must be set to the same value as windowCellsExponent for CA CFAR*/
