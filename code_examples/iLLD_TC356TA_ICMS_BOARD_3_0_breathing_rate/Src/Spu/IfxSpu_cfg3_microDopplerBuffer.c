/**********************************************************************************************************************
 * \file IfxSpu_cfg3_microDopplerBuffer.c
...
/*This SPU config copies the micro doppler frame and writes it with an address offset so that a circular buffer is set up*/

IfxSpu_ConfigSlot *IfxSpu_cfg3_microDopplerBuffer(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
    ...
    cfg->input.rm.baseAddress         = IFX_OFFSETOF(SPU_Rmem_Map_t, microDoppler[1][0][0]);
    ...
    cfg->input.rm.binLoopRepeat       = RADAR_NUM_SAMPLES/2*RADAR_NUM_VRX;
    cfg->input.rm.innerLoopRepeat     = 1;
    cfg->input.rm.outerLoopRepeat     = RADAR_NUM_MICRO_DOPPLER-1;

    cfg->input.rm.binLoopOffset       = bin_size_bytes;
    cfg->input.rm.innerLoopOffset     = 1;
    cfg->input.rm.outerLoopOffset     = bin_size_bytes * RADAR_NUM_SAMPLES/2*RADAR_NUM_VRX;
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        /*MATH2*/
        /*summation for copying 16-bit data*/
        pcfg0->csum.baseAddress       = IFX_OFFSETOF(SPU_Rmem_Map_t, microDoppler[0][0][0]);
        pcfg0->csum.mode              = IfxSpu_Summation_Mode_sumAnt;
        pcfg0->csum.antennaeToUse     = 0x01;
