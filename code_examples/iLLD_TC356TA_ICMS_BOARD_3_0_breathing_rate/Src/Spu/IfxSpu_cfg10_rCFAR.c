...
/*This SPU config performs CFAR on power spectrum along range dimension */

IfxSpu_ConfigSlot *IfxSpu_cfg10_rCFAR(IfxSpu_SlotConfig *cfg, uint32 slotNr, Ifx_SPU *spu)
{
   ...
   IfxSpu_setupLocalMax(slot, NULL_PTR); /* disabled with passing NULL_PTR */
    ...
    /** ------------ PASS 0 ------------ */
    {
        ...
        pcfg0->cfar.baseAddress = IFX_OFFSETOF(SPU_Rmem_Map_t, CFARRMap[0]);
        IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, (pcfg0->cfar.baseAddress & 0x1F) == 0); /* 32-byte alignment*/
        ...
        pcfg0->cfarCA.guardCells = 3;
        pcfg0->cfarCA.windowCellsExponent = 4;
        pcfg0->cfarCA.beta = 0x0800; /* threshold set to 2*/
        pcfg0->cfarCA.cashSubWindowExponent = 4;
