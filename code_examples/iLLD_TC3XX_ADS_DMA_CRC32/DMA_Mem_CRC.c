...
#define DMA_PFLASH_SRC_ADDR 0xA0018000             /* Starting address in PFLASH for CRC calculation    */
...
/* Variable to hold CRC variable calculated by DMA */
uint32 g_DMACrc;

/* Variable to hold CRC variable calculated by CPU CRC32 */
uint32 g_CpuCrc;
...
void initDMA(void)
{
    ...
    g_DMA.pSourceAddressForDmaTransfer      = (uint32 *) DMA_PFLASH_SRC_ADDR;
...
void runDMA(void)
{
    ...
    /* Get CRC over transferred data from the DMA module */
    g_DMACrc = IfxDma_getDataCRC(&MODULE_DMA, IfxDma_ChannelId_0);

    /* Calculate DMA over source data using CPU CRC32 */
    g_CpuCrc = 0;

    int i;
    uint32 *pSrc;

    pSrc = g_DMA.pSourceAddressForDmaTransfer;

    for(i = 0; i < DATA_ARRAY_LENGTH; i++)
    {
        g_CpuCrc = __crc32(g_CpuCrc, *pSrc);
        pSrc++;
    }

    /* Start verification of data content and check if CRC from CPU and DMA match*/
    if((verifyDMACopiedData() == TRUE) && (g_CpuCrc == g_DMACrc))
    {
