...
/** \brief Get pointer to the DTM (Dead-Time Module) structure for a specified PWM submodule and channel.
 * \param clusterSFR Pointer to the GTM cluster's SFR (Special Function Register) structure.
 * \param subModule GTM Sub Module
 * \param channel PWM channel number (0 to 15).
 ...
 * \param glbCtrl Pointer to the global control register of the PWM module.
 * \param upenMask Bitmask to enable or disable the synchronous update for specific channels.
 * \return None
 */
 ...
 * \param glbCtrl Pointer to the global control register.
 * \param upenMask Bitmask specifying which PWM channels to disable for synchronous updates.
 * \return None
 */
 ...
 * \param clusterSFR Pointer to the Cluster object.
 * \param subModule PWM submodule to configure (ATOM or TOM).
 * \param channel Channel index.
 ...
 * \param subModule ATOM/TOM
 * \param channel Channel index
 * \param enabled TRUE: Generate trigger FALSE: Forward trigger
 * \return None
 */
 ...
 * \param clusterSFR Pointer to the Cluster object
 * \param subModule PWM submodule selection ATOM/TOM
 * \param channel Channel index
...
void IfxGtm_Pwm_init(IfxGtm_Pwm *pwm, IfxGtm_Pwm_Channel *channels, IfxGtm_Pwm_Config *config)
{
    ...
    while (idx <= (uint8)config->channels[config->numChannels - 1u].timerCh)   /* Reset all channels in range even if not configured by user */
    {
        if (pwm->subModule == IfxGtm_Pwm_SubModule_atom)
        {
            pwm->clusterSFR->ATOM.AGC.GLB_CTRL.U = 1u << (IFX_GTM_CLS_ATOM_AGC_GLB_CTRL_RST_CH0_OFF + idx);
        }
        else
        {
            pwm->clusterSFR->TOM.TGC[IFXGTM_PWM_GET_TGC_INDEX(idx)].GLB_CTRL.U = 1u << (IFX_GTM_CLS_TOM_TGC_GLB_CTRL_RST_CH0_OFF + (idx & 0x7u));
        }
...
IFX_STATIC void IfxGtm_Pwm_updateGlobalControlUnitData(IfxGtm_Pwm *pwm)
{
    ...
    if (pwm->subModule == IfxGtm_Pwm_SubModule_atom)
    {
        ...
        pwm->globalControl.upenMask0     = (mask & 0xFFFFu) << (uint32)IFX_GTM_CLS_ATOM_AGC_GLB_CTRL_UPEN_CTRL0_OFF;
    ...
    else
    {
        ...
        if (IFXGTM_PWM_GET_TGC_INDEX(firstChannel) != IFXGTM_PWM_GET_TGC_INDEX(lastChannel))
        {
            ...
            pwm->globalControl.upenMask0     = (mask & 0xFFFFu) << (uint32)IFX_GTM_CLS_TOM_TGC_GLB_CTRL_UPEN_CTRL0_OFF;
...
void IfxGtm_Pwm_resetFastShutoff(IfxGtm_Pwm *pwm)
{
    ...
    for (idx = 0u; idx < pwm->numChannels; idx++)
    {
        ...
        Ifx_GTM_CLS_CDTM_DTM  *dtmSFR        = IfxGtm_Pwm_getDtmPtr(pwm->clusterSFR, pwm->subModule, channelIndex);
        IfxGtm_Dtm_Ch          dtmChannel    = (IfxGtm_Dtm_Ch)((uint8)((uint8)channelIndex & 0x3u));

        /* Write SHUT_OFF_RST_x bit in DTM CTRL2 to reset SHUTOFF_SYNC_x */
        IfxGtm_Dtm_resetShutoffSignal(dtmSFR, dtmChannel);
    }
}
...
void IfxGtm_Pwm_enableSelfResetOfFastShutoff(IfxGtm_Pwm *pwm)
{
    ...
    for (idx = 0u; idx < pwm->numChannels; idx++)
    {
        ...
        IfxGtm_Dtm_setShutoffUpdateMode(dtmSFR, dtmChannel, IFXGTM_DTM_UPD_MODE_SELF_RESET);
    }
}
