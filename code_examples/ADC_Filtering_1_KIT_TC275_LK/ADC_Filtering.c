...
channel g_chn[] = { {&IfxVadc_G4_7_P40_9_IN, (IfxVadc_ChannelResult) 0  },   /* AN39 pin    (Average filter channel)   */
...
void applyFiltering(void)
{
    ...
    VADC_G4_RCR0.B.DRCTR = DRCTR_DATA_REDUCTION;    /* Configure the Result Register 0 of Group 4 to accumulate 4 conversions */

    /* Apply a 3rd order Finite Impulse Response Filter (FIR) to the result register G4RES7 (AN38) */
    VADC_G4_RCR7.B.DMM = DMM_RESULT_FILTERING;      /* Set the Data Modification Mode bit field to Result filtering mode */
    VADC_G4_RCR7.B.DRCTR = DRCTR_FIR_COEFF;         /* Configure the Result Register 7 of Group 4 to apply a FIR filter (a=1, b=0, c=2) */

    /* Apply a 1st order Infinite Impulse Response Filter (IIR) to the result register G4RES15 (AN37) */
    VADC_G4_RCR15.B.DMM = DMM_RESULT_FILTERING;     /* Set the Data Modification Mode bit field to Result filtering mode */
    VADC_G4_RCR15.B.DRCTR = DRCTR_IIR_COEFF;        /* Configure the Result Register 15 of Group 4 to apply an IIR filter (a=3, b=4) */
}
